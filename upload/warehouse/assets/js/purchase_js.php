...
<script>
	...
	(function($) {
		...
		wh_init_ajax_search("supplier", "#supplier_code.ajax-search", undefined, admin_url+'warehouse/get_relation_data');
...
$('select[name="pr_order_id"]').on('change', function() {
	...
	$.get(admin_url+'warehouse/coppy_pur_request/'+pr_order_id + '/' + warehouse_id_m).done(function(response){
		...
		if(response){
			...
			$('input[name="currency_exchange_rate"]').val(response.currency_exchange_rate);

			init_goods_receipt_currency(response.currency);
		...
		if(pr_order_id != ''){
			...
			$.post(admin_url + 'warehouse/copy_pur_vender/'+pr_order_id).done(function(response){
				...
				$('.f_supplier_code').addClass('hide');
				$('.f_supplier_name').removeClass('hide');
				$('.f_supplier_name input[name="supplier_name"]').val(response_vendor.supplier_name);
				$('.i_supplier_code').removeClass('hide');
				$('.i_supplier_code').html(hidden_input('supplier_code', response_vendor.userid));
			...
			$('.f_supplier_code').removeClass('hide');
			$('.f_supplier_name').addClass('hide');

			$('.f_supplier_name input[name="supplier_name"]').val('');
			$('.i_supplier_code').addClass('hide');
			$('.i_supplier_code').html('');
...
function submit_form(save_and_send_request) {
	...
	if(check_warehouse_status == true){
		...
		if($('#add_goods_receipt').valid()){
			$('.add_goods_receipt_send').addClass("disabled");
			$('.add_goods_receipt').addClass("disabled");
...
function wh_init_ajax_search(type, selector, server_data, url) {
    var ajaxSelector = $('body').find(selector);

    if (ajaxSelector.length) {
        var options = {
            ajax: {
                url: (typeof (url) == 'undefined' ? admin_url + 'warehouse/get_relation_data' : url),
                data: function () {
                    var data = {};
                    data.type = type;
                    data.rel_id = '';
        ...
        console.log('ajaxSelector', ajaxSelector);

<script>
  ...
  (function($) {
    ...
    $('select[name="label_sizes"]').on('change', function () {
    "use strict";

      var label_size = $(this).val();
      if(label_size != ''){

        $.get(admin_url + 'warehouse/get_label_size/' +label_size, function (response1) {
          response1 = JSON.parse(response1);
        $('#table_commodity_list_print_barcode input[name="number_of_label_in_row"]').val(response1.number_of_label_in_row);
        $('#table_commodity_list_print_barcode input[name="number_of_row"]').val(response1.number_of_row);
        $('#table_commodity_list_print_barcode input[name="label_width"]').val(response1.width);
        $('#table_commodity_list_print_barcode input[name="label_height"]').val(response1.height);
        $('#table_commodity_list_print_barcode input[name="horizontal_distance"]').val(response1.width);
        $('#table_commodity_list_print_barcode input[name="vertical_distance"]').val(response1.height);
        $('#table_commodity_list_print_barcode input[name="distance_from_page_left"]').val(0);
        $('#table_commodity_list_print_barcode input[name="distance_from_the_top_of_the_page"]').val(0);
          
        });
      }
    });

    $('input[name="custom_printing"]').on('change', function () {
    "use strict";
      if ($('input[name="custom_printing"]').is(':checked')) {
        $('.custom_printing_hide').removeClass('hide');

        appValidateForm($('#item_print_barcode'), {
              label_sizes: 'required',
        }); 

      } else {
        $('.custom_printing_hide').addClass('hide'); 
        appValidateForm($('#item_print_barcode'), {
        }); 
      } 
    });

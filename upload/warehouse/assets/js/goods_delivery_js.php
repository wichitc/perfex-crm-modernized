...
console.log('total_discount_calculated=======0', total_discount_calculated);
console.log('additional_discount=======0', additional_discount);

  total = total - parseFloat(total_discount_calculated) - parseFloat(additional_discount);
...
function submit_form(save_and_send_request) {
  ...
  if(check_warehouse_status == true && check_quantity_status == true && check_available_quantity_status == true){
    ...
    if($('#add_goods_delivery').valid()){
      $('.add_goods_delivery_send').addClass("disabled");
      $('.add_goods_delivery').addClass("disabled");
...
function invoice_change(){
  ...
  $.post(admin_url + 'warehouse/copy_invoices/'+invoice_id).done(function(response){
    ...
    init_goods_delivery_currency(response.currency);

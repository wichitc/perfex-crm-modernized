<script>
  ...
  (function($) {
         ...
         if (this.getUploadingFiles().length === 0 && this.getQueuedFiles().length === 0) {
          ...
          if(response.add_variant == 'add_variant'){
            $.get(admin_url + 'warehouse/copy_product_image/' +response.id, function (response1) {
              ...
              var table_inventory = $('table.table-table_inventory');
              table_inventory.DataTable().ajax.reload(null, false);
    ...
    var table_inventory = $('table.table-table_inventory');
    var _table_api = initDataTable(table_inventory, admin_url+'warehouse/table_inventory', [0], [0], ProposalServerParams,  [1, 'desc']);
...
warehouse_type_value = warehouse_type;
  ...
  function staff_bulk_actions(){
    "use strict";
    $('#table_inventory_bulk_actions').modal('show');
  }
 ...
 function warehouse_delete_bulk_action(event) {
  ...
  if (confirm_delete()) {
    ...
    if(mass_delete == true || ( change_item_selling_price == true && selling_price != '') || ( change_item_purchase_price == true && purchase_price != '') || clone_items == true){
      ...
      var rows = $('#table-table_inventory').find('tbody tr');
      $.each(rows, function() {
      ...
      setTimeout(function() {
        $.post(admin_url + 'warehouse/warehouse_delete_bulk_action', data).done(function() {
          ...
          $('#table_inventory_bulk_actions').modal('hide');
...
/*print barcode*/
  function print_barcode_bulk_actions(){
    ...
    $("#table_inventory_print_barcode option:selected").prop("selected", false).change()
    $("table_inventory_print_barcode select[id='item_select_print_barcode']").selectpicker('refresh');

    $('#table_inventory_print_barcode').modal('show');
  }
    ...
    if(mass_delete == true){
      ...
      setTimeout(function() {
        $.post(admin_url + 'warehouse/warehouse_delete_bulk_action', data).done(function() {
          ...
          $('#table_inventory_print_barcode').modal('hide');
          alert_float('danger', data.responseText);
        });
      }, 200);
    }else{

...
if (isset($warehouse_ft)) {
	$where[] = 'AND ' . db_prefix() . 'inventory_manage.warehouse_id IN (' . implode(', ', $warehouse_ft) . ')';
}
...
$arr_article_out_of_stock = [];

foreach ($rResult as $aRow) {
	$product_inventory_quantity = 0;
	$current_quantity = (float) $aRow['inventory_number'];
	...
	if (($current_quantity == 0)) {
		...
		$arr_article_out_of_stock[] = $aRow['commodity_id'];
	...
	for ($i = 0; $i < count($aColumns); $i++) {
		...
		if ($aColumns[$i] == 'commodity_code') {
			...
			$str = '';
			$color = '';
			if ($aRow['expiry_date'] != '' && $aRow['expiry_date'] != '0000-00-00') {
				if (strtotime($aRow['expiry_date']) <= strtotime(date("Y-m-d"))) {
					$color = 'label-tab3';
				} else {
					$color = 'label-tab1';
				}
				...
				$quantity_by_warehouse = $aRow['inventory_number'];

				$str .= '<span class="label label-tag tag-id-1 ' . $color . '"><span class="tag">' . _d($aRow['expiry_date']) . ': ( ' . $quantity_by_warehouse . ' )</span><span class="hide">, </span></span>&nbsp';
			...
			$team = get_warehouse_name($aRow['iv_warehouse_id']);
			if ($team) {
				$quantity_by_warehouse = $aRow['inventory_number'];
				...
				if (!$check_hide_warehouse) {
					...
					$str .= '<span class="label label-tag tag-id-1"><span class="tag">' . $value . ': ( ' . $quantity_by_warehouse . ' )</span><span class="hide">, </span></span>';
					$_data .= $str;
				}
			...
			$_data = app_format_money((float) $aRow['rate'], $base_currency);
		} elseif ($aColumns[$i] == 'purchase_price') {
			$_data = app_format_money((float) $aRow['purchase_price'], $base_currency);

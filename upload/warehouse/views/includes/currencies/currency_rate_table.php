...
foreach ($rResult as $aRow) {
    ...
    $_data .= ' | <a href="'.admin_url('warehouse/delete_currency_rate/'.$aRow['id'].'').'" class="text-danger _delete">' . _l('delete') . '</a>';
    $_data .= '</div>'; 

    $row[] = $aRow['from_currency_name'].' '._l('wh_to').' '.$aRow['to_currency_name'].$_data;  

    $row[] = wh_app_format_number($aRow['to_currency_rate']);  

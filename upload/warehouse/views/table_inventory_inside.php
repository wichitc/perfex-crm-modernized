	...
	foreach ($rResult as $aRow) {
		...
		for ($i = 0; $i < count($aColumns); $i++) {
			...
			if ($aColumns[$i] == 'description') {
				...
				$_data = app_format_money((float)$aRow['purchase_price'], $base_currency_id);
			}elseif ($aColumns[$i] == 'rate') {
				$_data =  app_format_money((float)$aRow['rate'], $base_currency_id);
			}elseif ($aColumns[$i] == 'group_id') {
				/*get average price of inventory*/

				$_data =  app_format_money((float)$average_price_inventory['average_price_of_inventory'], $base_currency_id);

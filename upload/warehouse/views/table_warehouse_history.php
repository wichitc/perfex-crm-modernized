...
foreach ($rResult as $aRow) {
    if($aRow[db_prefix().'goods_transaction_detail.status'] == 3){
        $loss_adjustment_ids[] = $aRow['goods_receipt_id'];
    }
}

$arr_loss_adjustment = $this->ci->warehouse_model->get_loss_adjustment_type($loss_adjustment_ids);
    ...
    if($aRow['old_quantity'] != null && $aRow['old_quantity'] != ''){
     ...
     switch ($aRow[db_prefix().'goods_transaction_detail.status']) {
           ...
           case 2:
               ...
               break;
            case 3:
               if(isset($arr_loss_adjustment[$aRow['goods_receipt_id']])){
                 $row[] = _l($arr_loss_adjustment[$aRow['goods_receipt_id']]);
             }else{

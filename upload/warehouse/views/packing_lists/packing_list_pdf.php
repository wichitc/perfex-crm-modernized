...
$pdf_display_expiry_date = get_option('packing_list_pdf_display_expiry_date');
...
if($pdf_display_expiry_date == 0){
	$item_description_width += 8;
}

if($pdf_display_discount_percent == 0){
	$item_description_width += 7;
}
...
$table_font_size = 'font-size:'.($font_size + 1).'px;';
$items = '';
...
if($pdf_display_expiry_date == 1){
	$items.='<th width="8%" align="right" style="font-size:12px;">'. _l('expiry_date').'</th>';
}
...
if($pdf_display_subtotal == 1){
	$items .='<th width="10%" align="right" style="font-size:12px;">'. _l('subtotal').'</th>';
}
if($pdf_display_discount_percent == 1){
	$items .='<th width="7%" align="right" style="font-size:12px;">'. _l('discount').' (%)'.'</th>';
}
...
$only_item_name = get_option('packing_list_pdf_display_only_item_name');
...
foreach ($packing_list->packing_list_detail as $key => $packing_list_detail) {
	...
	$itemHTML .= '<td class="description" align="left;" width="'.$item_description_width.'%">';
			...
			if($only_item_name){
				$commodity_name = wh_get_item_variatiom($packing_list_detail['commodity_code'], true);
			}

			if (!empty($packing_list_detail['commodity_name'])) {
				$itemHTML .= '<span class="font-size:'.($font_size).'px;"><strong>'
				. $commodity_name
				. '</strong></span>';
			...
			if($pdf_display_expiry_date == 1){
				$itemHTML .= '<td align="right" width="8%">' . $packing_list_detail['expiry_date'] . '</td>';
			}
			...
			if($pdf_display_subtotal == 1){
				...
				$itemHTML .= '<td class="amount" align="right" width="10%">' . app_format_money($packing_list_detail['sub_total'], '') . '</td>';
			}
			
			// sub total
			if($pdf_display_discount_percent == 1){

				$itemHTML .= '<td class="amount" align="right" width="7%">' . $packing_list_detail['discount'] . '</td>';
			}
...
$items.= '</tbody>
		...
		<td align="right" width="15%">' . app_format_money($packing_list->subtotal, $packing_list->currency) . '</td>
		</tr>';
		...
		<td align="right" width="15%">' . app_format_money($discount_total, $packing_list->currency) . '</td>
		</tr>';

		$tbltotal .= '<tr>
		<td align="right" width="85%"><strong>' . _l('wh_shipping_fee') . '</strong></td>
		<td align="right" width="15%">' . app_format_money($packing_list->shipping_fee, $packing_list->currency) . '</td>
		</tr>';

		$tbltotal .= '
		<tr style="background-color:#f0f0f0;">
		<td align="right" width="85%"><strong>' . _l('total_money') . '</strong></td>
		<td align="right" width="15%">' . app_format_money($packing_list->total_after_discount, $packing_list->currency) . '</td>
		</tr>';

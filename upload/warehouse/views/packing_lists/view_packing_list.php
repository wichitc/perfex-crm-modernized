									...
									<table class="table items items-preview estimate-items-preview" data-type="estimate">
										<thead>
											<tr>
												...
												<th align="right" colspan="1" class="<?php if(get_option('packing_list_expiry_date') == 0){ echo " hide";} ?>"><?php echo _l('expiry_date') ?></th>
										...
										<tbody class="ui-sortable">
											...
											foreach ($packing_list_detail as $delivery => $packing_list_value) {
												...
												<tr>
													...
													<td class="text-right <?php if(get_option('packing_list_expiry_date') == 0){ echo " hide";} ?>"><?php echo html_entity_decode($packing_list_value['expiry_date'] ?? '') ?></td>
										...
										<table class="table text-right">
											<tbody>
												<tr id="subtotal">
													...
													<td><?php echo app_format_money((float)$packing_list->subtotal, $currency); ?></td>
												</tr>
														...
														<div class="col-md-3 text-center">
															...
															<?php if($value['approve'] == 1){ 
																...
																<?php if (file_exists(WAREHOUSE_PACKING_LIST_MODULE_UPLOAD_FOLDER . $packing_list->id . '/signature_'.$value['id'].'.png') ){ ?>
																	...
																	<img src="<?php echo site_url('modules/warehouse/uploads/packing_lists/'.$packing_list->id.'/signature_'.$value['id'].'.png'); ?>" class="img-width-height">
								...
								<div class="pull-right">
									...
									if($packing_list->approval != 1 && ($check_approve_status == false ))
										...
										if(isset($check_approve_status['staffid'])){
											?>
											<?php 
              if((in_array(get_staff_user_id(), $check_approve_status['staffid']) && $packing_list->approval == 0 && !isset($check_approve_status['action']) && !in_array(get_staff_user_id(), $get_staff_sign) ) || (  in_array(get_staff_user_id(), $check_approve_status['staffid']) && $packing_list->approval == 0 && isset($check_approve_status['action']) && $check_approve_status['action'] == 'approve') ){ ?>
												<div class="btn-group" >
												...
												<?php
                 if((in_array(get_staff_user_id(), $check_approve_status['staffid']) && in_array(get_staff_user_id(), $get_staff_sign) && $packing_list->approval == 0 && !isset($check_approve_status['action'])) || (in_array(get_staff_user_id(), $check_approve_status['staffid']) && in_array(get_staff_user_id(), $get_staff_sign) && $packing_list->approval == 0 && isset($check_approve_status['action']) && $check_approve_status['action'] == 'sign')  ){ ?>
							...
							<div class="modal-content">
								...
								<div class="modal-body">
									...
									<div id="upload_sign" class="mbot15 hide">
										...
										<?php echo form_hidden('approve_rel_id', $packing_list->id) ?>
										<?php echo form_hidden('approve_rel_type', '5') ?>

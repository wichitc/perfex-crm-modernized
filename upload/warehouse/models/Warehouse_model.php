...
class Warehouse_model extends App_Model {
	 ...
	 * @return array or object
	 */
	public function get_warehouse($id = false, $setting = false, $where = []) {
	 ...
	 * @return array
	 */
	public function get_commodity_code_name($item_id = '') {
		if($item_id != ''){
			$arr_value = $this->db->query('select * from ' . db_prefix() . 'items where id = '.$item_id.' AND active = 1 AND id not in ( SELECT distinct parent_id from '.db_prefix().'items WHERE parent_id is not null AND parent_id != "0" ) order by id asc')->result_array();
	...
	public function delete_commodity($id) {
		...
		if ($this->db->affected_rows() > 0) {
			hooks()->do_action('wh_delete_item', $id);
			return true;
		}
		return false;
	...
	public function add_goods_receipt($data, $id = false) {
		...
		if (isset($data['custom_fields'])) {
            ...
            foreach ($custom_fields as $key => $value) {
                if(preg_match('/^iv_receipt/', $key)){
                    $inventory_receipt_custom_fields = [];
                    $inventory_receipt_custom_fields[$key] = $value;
                }
            }
		...
		if (get_status_modules_wh('purchase')) {
			if($data['supplier_code'] != ''){
				...
				$client                = $this->purchase_model->get_vendor($data['supplier_code']);
				if($client){
					if(is_array($client)){
						if(count($client) > 0){
							$data['supplier_name'] = isset($client[0]['company']) ? $client[0]['company'] : (isset($client[0]->company) ? $client[0]->company : '');
						}
					} else {
						$data['supplier_name'] = isset($client->company) ? $client->company : '';
					}
				}
		...
		if (isset($insert_id)) {
			...
			if (isset($inventory_receipt_custom_fields)) {
                handle_custom_fields_post($insert_id, $inventory_receipt_custom_fields);
            }
	...
	public function get_pur_request($pur_order, $warehouse_id) {
		...
		if($get_pur_order){
			if(get_option('goods_receipt_do_not_convert_to_base_currency') == 0){
				if($get_pur_order->currency_rate != null){
					$currency_rate = $get_pur_order->currency_rate;
				}
			}
			if(get_option('goods_receipt_do_not_convert_to_base_currency') == 1){
				$currency = (float)$get_pur_order->to_currency;
				$currency_exchange_rate = (float)$get_pur_order->currency_rate;
			}
		}
		...
		$arr_pur_resquest[] = $currency;
		$arr_pur_resquest[] = $currency_exchange_rate;
	...
	public function get_approve_setting($type, $status = '1', $json_decode = true) {
		...
		$approval_setting = $this->db->get('wh_approval_setting')->row();
		if ($approval_setting) {
			if($json_decode){
	...
	public function update_approve_request($rel_id, $rel_type, $status) {
		...
		//case 1: stock_import
			...
			// //update history stock, inventoty manage after staff approved
				...
				foreach ($goods_receipt_detail as $key => $goods_receipt_detail_value) {
					...
					if($from_po && $goods_receipt->currency != 0 && $goods_receipt->currency != $base_currency && (float)($goods_receipt->currency_exchange_rate) != 0){
						$goods_receipt_detail_value['unit_price'] = round((float)$goods_receipt_detail_value['unit_price'] / (float)($goods_receipt->currency_exchange_rate), 5);
					}
				...
				if($from_po == true && $flag_update_status_po == true){
					if (get_status_modules_wh('purchase')) {
						// change PO delivery status to Completely Delivered
				...
				}elseif($from_po == true && $flag_update_status_po == false){
					if (get_status_modules_wh('purchase')) {
						// change PO delivery status to Partially Delivered
						if ($this->db->field_exists('delivery_status' ,db_prefix() . 'pur_orders')) { 
							$this->db->where('id', $goods_receipt->pr_order_id);
							$this->db->update(db_prefix() . 'pur_orders', ['delivery_status' => 3]);
						}
					}
				}
			...
			//update history stock, inventoty manage after staff approved
				...
				//update Without checking warehouse				
					if($this->check_item_without_checking_warehouse($goods_delivery_detail_value['commodity_code']) == true){
						...
						if($goods_delivery->currency != 0 && $goods_delivery->currency != $base_currency && (float)($goods_delivery->currency_exchange_rate) != 0){
							$goods_delivery_detail_value['unit_price'] = round((float)$goods_delivery_detail_value['unit_price'] * (float)($goods_delivery->currency_exchange_rate), 5);
						}
		...
		//internal delivery note
			...
			if((int)$status == 1){
	            ...
	            hooks()->do_action('after_internal_delivery_approve', $rel_id);
			}
	 ...
	 * @return  pdf view
	 */
	function stock_import_pdf($purchase, $goods_receipt) {
		return app_pdf('purchase', module_dir_path(WAREHOUSE_MODULE_NAME, 'libraries/pdf/Purchase_pdf.php'), $purchase, $goods_receipt);
	}
	...
	public function get_stock_import_pdf_html($goods_receipt_id) {
		...
		$tax_data = $this->get_html_tax_receip($goods_receipt_id, $currency);
		...
		$html .= '<table class="table invoice-items-table items table-main-invoice-edit has-calculations no-mtop" cellpadding="2">
		<thead>
		<tr height="30" bgcolor="' . get_option('pdf_table_heading_color') . '" style="color:' . get_option('pdf_table_heading_text_color') .';'. $table_font_size. '; ">
		<th style="width:5%;">'. _l('_order').'</th>';
		$commodity_code_width = '22%';
		if($display_product_image){
			$html .= '<th style="width:6%;">'. _l('_images').'</th>';
			$commodity_code_width = '16%';
		}

		$html .= '<th class="" style="width:'.$commodity_code_width.';">' . _l('commodity_code') . '</th>
		<th class="" style="width:13%;">' . _l('warehouse_name') . '</th>
		<th class="" style="width:7%;">' . _l('unit_name') . '</th>
		<th class="" style="width:7%;" align="right">' . _l('quantity') . '</th>
		<th class="" style="width:8%;" align="right">' . _l('unit_price') . '</th>
		<th class="" style="width:10%;" align="right">' . _l('total_money') . '</th>
		<th class="" style="width:9%;" align="right">' . _l('tax_money') . '</th>
		<th class="" style="width:10%;">' . _l('lot_number') . '</th>
		<th class="" style="width:9%;">' . _l('expiry_date') . '</th>
		</tr>
		</thead>
		...
		foreach ($goods_receipt_detail as $receipt_key => $receipt_value) {
			...
			$item_image = '';
			if($display_product_image){
				$item_image = $this->get_item_image($receipt_value['commodity_code'], 'height="52" width="55"');
			}

			$key = $receipt_key+1;

			$html .= '<tr style="'. $table_font_size. '; ">';
			$html .= '<td align="left" style="width:5%;"><b>' . $key . '</b></td>';
			if($display_product_image){
				$html .= '<td style="width:6%;">' . $item_image.'</td>';
			}
			$html .= '<td style="width:'.$commodity_code_width.';"><b>' . $commodity_name.'</b></td>
			<td style="width:13%;">' . $warehouse_code . '</td>
			<td style="width:7%;">' . $unit_name . '</td>
			<td style="width:7%;" align="right">' . $quantities . '</td>
			<td style="width:8%;" align="right">' . app_format_money((float) $unit_price, $currency) . '</td>
			<td style="width:10%;" align="right">' . app_format_money((float) $goods_money, $currency) . '</td>
			<td style="width:9%;" align="right">' . app_format_money((float) $tax_money, $currency) . '</td>
			<td style="width:10%;">' . $lot_number . '</td>
			<td style="width:9%;">' . _d($expiry_date) . '</td>
			</tr>';
			...
			if(new_strlen($receipt_value['serial_number'] ?? '') > 0){
				...
				foreach ($arr_serial_numbers as $serial_number_value) {
					...
					$serial_number_html .= '<tr style="'.$table_font_size.'"><td width="5%"><b>' . $serial_number_index . '</b></td>
		...
		if(new_strlen($serial_number_html ?? '') > 0){
			...
			<thead>
			<tr height="40" bgcolor="' . get_option('pdf_table_heading_color') . '" style="color:' . get_option('pdf_table_heading_text_color') .';'. $table_font_size. '; ">
		...
		<td class="text_right">' . app_format_money((float) $goods_receipt->total_goods_money, $currency) . '</td>
		</tr>
		...
		<td class="text_right">' . app_format_money((float) $goods_receipt->value_of_inventory, $currency) . '</td>
		</tr>';
		...
		<td class="text_right">' . app_format_money((float) $goods_receipt->total_tax_money, $currency) . '</td>
		</tr>';
		...
		<td class="text_right">' . app_format_money((float) $goods_receipt->total_money, $currency) . '</td>
		</tr>
	...
	public function send_mail($data ,$staffid = ''){
		...
		// case '1 : stock_import':
			...
			$link = 'warehouse/manage_purchase/' . $data['rel_id'];
			break;
			case '2':
			...
			$link = 'warehouse/manage_delivery/' . $data['rel_id'];
			break;
			case '3':
	...
	public function add_goods_delivery($data, $id = false) {
		...
		if (isset($data['custom_fields'])) {
            $custom_fields = $data['custom_fields'];
            foreach ($custom_fields as $key => $value) {
                if(preg_match('/^iv_delivery/', $key)){
                    $inventory_delivery_custom_fields = [];
                    $inventory_delivery_custom_fields[$key] = $value;
                }
            }
            unset($data['custom_fields']);
        }
		...
		if (isset($insert_id)) {
			...
			if (isset($inventory_delivery_custom_fields)) {
                handle_custom_fields_post($insert_id, $inventory_delivery_custom_fields);
            }
	...
	public function get_vendor_ajax($pur_orders_id) {
		...
		$data['supplier_name'] 		= $result_array->company;
	 ...
	 * @return pdf view
	 */
	public function stock_export_pdf($delivery, $goods_delivery) {
		return app_pdf('delivery', module_dir_path(WAREHOUSE_MODULE_NAME, 'libraries/pdf/Delivery_pdf.php'), $delivery, $goods_delivery);
	}
	...
	public function get_stock_export_pdf_html($goods_delivery_id) {
		...
		$company_name = get_option('invoice_company_name');
		$address = get_option('invoice_company_address');
		$tax_data = $this->get_html_tax_delivery($goods_delivery_id, $currency);
		...
		$html .= '<th  class="" style="width:'.$commodity_code_width.';" align="left">' ._l('commodity_name').'</th>';
		if($warehouse_lotnumber_bottom_infor_option == 1){
			$html .= '<th  class="" style="width:13%;" align="left">' ._l('warehouse_name').'</th>';
		}
		$html .= '<th  class="" style="width:7%;" align="right">' . _l('quantity') . '</th>
		<th  class="" style="width:7%;" align="right">' . _l('delivered') . '</th>';
		if(get_warehouse_option('goods_delivery_pdf_display_outstanding') == 1){
			$html .= '<th  class="" style="width:7%;" align="right">' . _l('outstanding') . '</th>';
		}
		$html .= '<th  class="" style="width:8%;" align="right">' . _l('unit_price') . '</th>';
		if($warehouse_lotnumber_bottom_infor_option == 1){
			$html .= '<th  class="" style="width:8%;" align="left">' ._l('lot_number').'</th>';
		}
		$html .= '<th  class="" style="width:8%;" align="right">' . _l('subtotal') . '</th>
		<th  class="" style="width:8%;" align="right">' . _l('subtotal_after_tax') . '</th>
		<th  class="" style="width:8%;" align="right">' . _l('total_money') . '</th>
		</tr>
		</thead>
		<tbody class="tbody-main">';
		$subtotal = 0 ;
		...
		foreach ($goods_delivery_detail as $delivery_key => $delivery_value) {
			...
			$item_image = '';
			if($display_product_image){
				$item_image = $this->get_item_image($delivery_value['commodity_code'], 'height="52" width="55"');
			}

			$html .= '<tr style="'. $table_font_size. '; ">';
			$html .= '<td align="left" style="width:4%;"><b>' . (float)$item_order . '</b></td>';
			if($display_product_image){
				$html .= '<td style="width:6%;">' . $item_image.'</td>';
			}
			$html .= '<td class="" style="width:'.$commodity_code_width.';" align="left"><b>' . $get_commodity_name . '</b></td>';
			if($warehouse_lotnumber_bottom_infor_option == 1){
				$html .= '<td class="" style="width:13%;" align="left"><b>' .$warehouse_name. '</b></td>';
			}
			$html .= '<td class="" style="width:7%;" align="right"><b>' . $quantities .' '.$unit_name. '</b></td>
			<td class="" style="width:7%;" align="right"><b>' . $quantities .' '.$unit_name. '</b></td>';
			if(get_warehouse_option('goods_delivery_pdf_display_outstanding') == 1){
				$html .= '<td class="" style="width:7%;" align="right"><b>0.0</b></td>';
			}

			if(get_warehouse_option('goods_delivery_pdf_display') == 1){
				$html .= ' <td class="" style="width:8%;" align="right"><b>' . app_format_money((float) $unit_price, '') . '</b></td>';
				if($warehouse_lotnumber_bottom_infor_option == 1){
					$html .= '<td class="" style="width:8%;" align="right"><b>' .$lot_number. '</b></td>';
				}
				$html .= '<td class="" style="width:8%;" align="right"><b>' . app_format_money((float) $item_subtotal, '') . '</b></td>
				<td class="" style="width:8%;" align="right"><b>' . app_format_money((float) $total_money, '') . '</b></td>
				<td class="" style="width:8%;" align="right"><b>' . app_format_money((float) $total_after_discount, '') . '</b></td>';

			}else{
				$html .= '<td class="" style="width:8%;" align="right"><b></b></td>';
				if($warehouse_lotnumber_bottom_infor_option == 1){
					$html .= '<td class="" style="width:8%;" align="right"><b>' .$lot_number. '</b></td>';
				}
				$html .= '<td class="" style="width:8%;" align="right"><b></b></td>
				<td class="" style="width:8%;" align="right"><b></b></td>
				<td class="" style="width:8%;" align="right"><b></b></td>';
		...
		foreach ($goods_delivery_detail as $value) {
			if(isset($new_goods_delivery_detail[$value['commodity_code'].'_'.$value['warehouse_id']])){
				...
				$new_goods_delivery_detail[$value['commodity_code'].'_'.$value['warehouse_id']]['commodity_name'] = $value['commodity_name'];
				$new_goods_delivery_detail[$value['commodity_code'].'_'.$value['warehouse_id']]['quantities'] = (float)$value['quantities'];
				$new_goods_delivery_detail[$value['commodity_code'].'_'.$value['warehouse_id']]['unit_price'] = (float)$value['unit_price'];
				$new_goods_delivery_detail[$value['commodity_code'].'_'.$value['warehouse_id']]['total_money'] = (float)$value['total_money'];
				$new_goods_delivery_detail[$value['commodity_code'].'_'.$value['warehouse_id']]['discount'] = (float)$value['discount'];
		...
		<thead>
		<tr height="30" bgcolor="' . get_option('pdf_table_heading_color') . '" style="color:' . get_option('pdf_table_heading_text_color') .';'. $table_font_size. '; ">
		<th  class="" style="width:5%;" align="left"><b>#</b></th>';
		$commodity_code_width = 22;
		if($display_product_image){
			$html .= '<th style="width:6%;">'. _l('_images').'</th>';
			$commodity_code_width = 16;
		...
		if($warehouse_lotnumber_bottom_infor_option != 1){
			$commodity_code_width += 13;
		}
		if(get_warehouse_option('goods_delivery_pdf_display_outstanding') != 1){
			$commodity_code_width += 8;
		}

		$commodity_code_width = $commodity_code_width.'%';
		...
		<th  class="" style="width:7%;" align="right">' . _l('delivered') . '</th>';
		if(get_warehouse_option('goods_delivery_pdf_display_outstanding') == 1){
			$html .= '<th  class="" style="width:8%;" align="right">' . _l('outstanding') . '</th>';
		}
		$html .= '<th  class="" style="width:10%;" align="right">' . _l('unit_price') . '</th>';
		
		$html .= '<th  class="" style="width:9%;" align="right">' . _l('subtotal') . '</th>
		<th  class="" style="width:10%;" align="right">' . _l('subtotal_after_tax') . '</th>
		<th  class="" style="width:9%;" align="right">' . _l('total_money') . '</th>
		...
		foreach ($new_goods_delivery_detail as $delivery_key => $delivery_value) {
			...
			$html .= '<tr style="'. $table_font_size. '; ">';
			$html .= '<td class="" style="width:5%;"><b>' . (float)$item_order . '</b></td>';
			if($display_product_image){
				$html .= '<td style="width:6%;">' . $item_image.'</td>';
			}
			$html .= '<td class="" style="width:'.$commodity_code_width.';"><b>' . $get_commodity_name . '</b></td>';
			if($warehouse_lotnumber_bottom_infor_option == 1){
				$html .= '<td class="" style="width:13%;"><b>' .$warehouse_name. '</b></td>';
			}
			$html .= '<td class="" style="width:7%;"><b>' . $quantities .' '.$unit_name. '</b></td>
			<td class="" style="width:7%;" align="right"><b>' . $quantities .' '.$unit_name. '</b></td>';
			if(get_warehouse_option('goods_delivery_pdf_display_outstanding') == 1){
				$html .= '<td class="" style="width:8%;" align="right"><b>0.0</b></td>';
			}

			if(get_warehouse_option('goods_delivery_pdf_display') == 1){
				$html .= ' <td class="" style="width:10%;" align="right"><b>' . app_format_money((float) $unit_price, '') . '</b></td>';
				
				$html .= '<td class="" style="width:9%;" align="right"><b>' . app_format_money((float) $item_subtotal, '') . '</b></td>
				<td class="" style="width:10%;" align="right"><b>' . app_format_money((float) $total_money, '') . '</b></td>
				<td class="" style="width:9%;" align="right"><b>' . app_format_money((float) $total_after_discount, '') . '</b></td>';

			}else{
				$html .= '<td style="width:10%;" align="right"><b></b></td>';
				if($warehouse_lotnumber_bottom_infor_option == 1){
					$html .= '<td class=""><b>' .$lot_number. '</b></td>';
				}
				$html .= '<td style="width:9%;" align="right"><b></b></td>
		...
		if(get_warehouse_option('goods_delivery_pdf_display') == 1){
			...
			<td class="text_right">' . app_format_money((float) $subtotal, $currency) . '</td>
			</tr>';
			...
			<td class="text_right">' . app_format_money((float) $total_discount, $currency) . '</td>
			</tr>
			<tr>
			<td ></td>
			<td ></td>
			...
			<td class="text_right">' . app_format_money((float) $shipping_fee, $currency) . '</td>
			</tr>
			<tr>
			<td ></td>
			<td ></td>
			<td ></td>
			<td class="text_left"><b>' . _l('total_money') . '</b></td>
			<td class="text_right">' . app_format_money((float) $after_discount, $currency) . '</td>
			</tr>
			</tbody>
			</table>
			<br><br><br>
	...
	public function get_stock_summary_report($data, $excel = false) {
		...
		foreach ($commodity_lists as $commodity_list_key => $commodity_list) {
			...
			<td class="bor_alir">' . app_format_money((float) $stock_opening_amount, $base_currency_id) . '</td>
			<td class="bor_alir">' . $import_period_quatity . '</td>
			<td class="bor_alir">' . app_format_money((float) $import_period_amount, $base_currency_id) . '</td>
			<td class="bor_alir">' . $export_period_quatity . '</td>
			<td class="bor_alir">' . app_format_money((float) $export_period_amount, $base_currency_id) . '</td>
			<td class="bor_alir">' . $closing_quatity . '</td>
			<td class="bor_r">' . app_format_money((float) $closing_amount, $base_currency_id) . '</td>
			</tr>';
		}
		$html .= '<tr>
		<th  colspan="4" class="th_stk_style">' . _l('total') . ' : </th>
		<th  colspan="1" class="th_stk_style">' . $total_opening_quatity . '</th>
		<th  colspan="1" class="th_stk_style">' . app_format_money((float) $total_opening_amount, $base_currency_id) . '</th>
		<th  colspan="1" class="th_stk_style">' . $total_import_period_quatity . '</th>
		<th  colspan="1" class="th_stk_style">' . app_format_money((float) $total_import_period_amount, $base_currency_id) . '</th>
		<th  colspan="1" class="th_stk_style">' . $total_export_period_quatity . '</th>
		<th  colspan="1" class="th_stk_style">' . app_format_money((float) $total_export_period_amount, $base_currency_id) . '</th>
		<th  colspan="1" class="th_stk_style">' . $total_closing_quatity . '</th>
		<th  colspan="1" class="th_st_spe">' . app_format_money((float) $total_closing_amount, $base_currency_id) . '</th>
		</tr>
		</tbody>
		</table>
		</div>';
	...
	public function update_commodity_one_item($data, $id) {
		...
		hooks()->do_action('wh_item_updated', $id);
	...
	public function add_loss_adjustment($data) {
		...
		if (isset($data['acc_class'])) {
			$data_add['acc_class'] = $data['acc_class'];
		}
	...
	public function get_inventory_valuation_report_view($data, $excel = false) {
		...
		foreach ($commodity_lists as $commodity_list_key => $commodity_list) {
			...
			<td class="td_style_r">' . app_format_money((float)$commodity_list['rate'] , $base_currency_id). '</td>
			<td class="td_style_r">' . app_format_money((float)$commodity_list['purchase_price'] , $base_currency_id). '</td>
			<td class="td_style_r">' . app_format_money((float) ($closing_quatity * $commodity_list['rate']), $base_currency_id) . '</td>
			<td class="td_style_r">' . app_format_money((float) ($closing_amount), $base_currency_id) . '</td>
			<td class="td_style_r'.$text_danger.'">' . app_format_money((float) ((float) $closing_quatity * $commodity_list['rate'] - (float)$closing_amount), $base_currency_id) . '</td>
			</tr>';
		}
		$html .= '<tr>
		...
		<th colspan="1" class="td_text_r">' . app_format_money((float) ($total_amount_sold), $base_currency_id) . '</th>
		<th colspan="1" class="td_text_r">' . app_format_money((float) ($total_amount_purchased), $base_currency_id) . '</th>
		<th colspan="1" class="td_text_r">' . app_format_money((float) ($total_expected_profit), $base_currency_id) . '</th>
		</tr>
		</tbody>
		</table>
		</div>
	...
	public function generate_commodity_barcode() {
		...
		$password = $this->ean13_check_digit($password);
		return $password;
	}
	...
	public function delete_goods_receipt($id) {
        ...
        $this->db->where('relid', $id);
        $this->db->where('fieldto', 'iv_receipt');
        $this->db->delete(db_prefix() . 'customfieldsvalues');
        if ($this->db->affected_rows() > 0) {
            $affected_rows++;
        }
	...
	public function delete_goods_delivery($id) {
		...
		// Delete the custom field values
        $this->db->where('relid', $id);
        $this->db->where('fieldto', 'iv_delivery');
        $this->db->delete(db_prefix() . 'customfieldsvalues');
        if ($this->db->affected_rows() > 0) {
            $affected_rows++;
        }
    ...
    public function update_goods_delivery($data, $id = false) {
    	...
    	if (isset($inventory_delivery_custom_fields)) {
            if(handle_custom_fields_post($goods_delivery_id, $inventory_delivery_custom_fields)) {
                $results++;
            }
        }
	...
	public function update_goods_receipt($data, $id = false) {
		...
		if (isset($inventory_receipt_custom_fields)) {
            if(handle_custom_fields_post($goods_receipt_id, $inventory_receipt_custom_fields)) {
                $results++;
            }
        }
    ...
    public function auto_create_goods_receipt_with_purchase_order($data)
    {
    	...
    	foreach ($results as $key => $value) {
    		...
    		$value['into_money'] = $value['unit_price'] * (float)$value['quantities'];
    ...
    public function auto_create_goods_delivery_with_invoice($invoice_id, $invoice_update='')
    {
    	...
    	if($invoice_value){
    		...
    		$invoice_currency_name = get_currency_name($invoice_value->currency);
    		$currency_exchange_rate_temp = wh_get_currency_rate($invoice_currency_name, $base_currency_name);
    		if(get_option('goods_receipt_do_not_convert_to_base_currency') == 0){
    			if((int)$invoice_value->currency != (int)$currency){
					$currency_rate = (float)$currency_exchange_rate_temp;
    			}
			}
			if(get_option('goods_receipt_do_not_convert_to_base_currency') == 1){
				$currency = (float)$invoice_value->currency;
				$currency_exchange_rate = (float)$currency_exchange_rate_temp;
			}
    		...
    		$data['sub_total'] 	= ((float)$invoice_value->subtotal * $currency_rate);

    		$data['total_money'] 	= ((float)$invoice_value->subtotal * $currency_rate) + ((float)$invoice_value->total_tax * $currency_rate);
			$data['total_discount'] = ((float)$invoice_value->discount_total * $currency_rate);
    		$data['after_discount'] 	= ((float)$invoice_value->total * $currency_rate);
    ...
    public function add_goods_delivery_from_invoice($data_insert, $invoice_id ='')
    {
    	...
    	if($flag_export_warehouse == 1){
    		...
    		foreach ($goods_delivery_detail as $goods_delivery_detail_value) {
    			...
    			if($this->check_item_without_checking_warehouse($goods_delivery_detail_value['commodity_code']) == true){
					// $this->add_inventory_manage($goods_delivery_detail_value, 2);
    				$this->add_inventory_from_invoices($goods_delivery_detail_value, $insert_id);
    			}
     ...
     * @param array $data 
     */
    public function add_inventory_from_invoices($data, $_goods_delivery_id)
    {		
		...
		$goods_delivery = $this->get_goods_delivery($_goods_delivery_id);
		if ($goods_delivery->currency != 0 && $goods_delivery->currency != $base_currency && (float) ($goods_delivery->currency_exchange_rate) != 0) {
			$data['unit_price'] = round((float) $data['unit_price'] * (float) ($goods_delivery->currency_exchange_rate), 5);
		}
    ...
    public function copy_invoice($invoice_id)
    {
    	...
    	$data_insert['status'] = $status;
    	$data_insert['currency'] = $currency;
    	$data_insert['currency_exchange_rate'] = $currency_exchange_rate;
	...
	public function revert_inventory_manage($data, $status, $invoice = false)
	{
    	...
    	// status '1:Goods receipt note 2:Goods delivery note',
			...
			$this->db->where('warehouse_id', $data['warehouse_id']);
			$this->db->where('commodity_id', $data['commodity_code']);
			if (!is_null($data['expiry_date']) && $data['expiry_date'] != '') {
				$this->db->where('expiry_date', $data['expiry_date']);
			}
			if (!is_null($data['lot_number']) && $data['lot_number'] != '') {
				$this->db->where('lot_number', $data['lot_number']);
			}
    ...
    public function update_goods_delivery_approval($data, $id = false)
    {
		...
		if($results > 0){
			hooks()->do_action('after_wh_goods_delivery_approval_updated', $data['id']);
			return true;
		}
		return false;
    }
   ...
   public function update_po_detail_quantity($po_id, $goods_receipt_detail)
    ...
    public function goods_delivery_get_pur_order($pur_order) {
    	...
    	if(count($arr_results) > 0){
    		...
    		foreach ($arr_results as $key => $value) {
    			$unit_price = round((float)$value['rate']/$currency_rate, 5);
    			...
    			$total_goods_money = (float)$value['quantities']*(float)$unit_price;
    	...
    	$arr_pur_resquest['additional_discount'] = (float)$additional_discount;
    	$arr_pur_resquest['currency'] = $currency;
    	$arr_pur_resquest['currency_exchange_rate'] = $currency_exchange_rate;
     ...
     * @return [type]           
     */
	public function stock_internal_delivery_pdf($internal, $internal_delivery) {
		return app_pdf('internal', module_dir_path(WAREHOUSE_MODULE_NAME, 'libraries/pdf/Internal_pdf.php'), $internal, $internal_delivery);
	}
    ...
    {	
		...
		<div class="col-md-8">
			...
			<span class="pull-left fa fa-question-circle" data-toggle="tooltip" title="" data-original-title="'. _l('wh_variation_tooltip').'"></span>
     ...
     */
    public function get_html_tax_receip($id, $currency = 0){
        $html = '';
        $preview_html = '';
        $html_currency = '';
        $pdf_html = '';
    ...
    public function create_goods_receipt_row_template($warehouse_data = [], $name = '', $commodity_name = '', $warehouse_id = '', $quantities = '', $unit_name = '', $unit_price = '', $taxname = '', $lot_number = '', $date_manufacture = '', $expiry_date = '', $commodity_code = '', $unit_id = '', $tax_rate = '', $tax_money = '', $goods_money = '', $note = '', $item_key = '', $sub_total = '', $tax_name = '', $tax_id = '', $is_edit = false, $serial_number = '') {
		...
		$display_product_image = get_option('display_product_image_receipt_delivery_pdf');
		if($display_product_image){
			$item_image = $this->get_item_image($commodity_code);
			if($item_image != ''){
				$item_image = '<div class="col-md-3 no-padding-left">'.$item_image.'</div>';
			}
		}
		...
		if($display_product_image && isset($item_image) && $item_image != ''){
			$row .= '<td class="">'.$item_image . '<div class="col-md-9 no-padding-left">'.render_textarea($name_commodity_name, '', $commodity_name, ['rows' => 2, 'placeholder' => _l('item_description_placeholder'), 'readonly' => true] ) . '</div></td>';
		}else{
	 ...
	 * @return [type]     
	 */
	public function get_html_tax_delivery($id, $currency = 0){
        $html = '';
        $html_currency = '';
        $preview_html = '';
        $pdf_html = '';
	...
	public function create_packing_list_row_template( $delivery_detail_id = '', $name = '', $commodity_name = '', $quantities = '', $unit_name = '', $unit_price = '', $taxname = '',  $commodity_code = '', $unit_id = '', $tax_rate = '', $total_amount = '', $discount = '', $discount_total = '', $total_after_discount = '', $sub_total = '', $tax_name = '', $tax_id = '', $item_key = '',$is_edit = false, $max_qty = false, $serial_number = '', $expiry_date = '') {
		...
		$expiry_date_hide = '';
		if(get_option('packing_list_expiry_date') == 0){
		...
		$row .= '<td class="rate">' . render_input($name_unit_price, '', $unit_price, 'number', $array_rate_attr) . '</td>';
		$row .= '<td class="expiry_date '.$expiry_date_hide.'">' . render_input($name_expiry_date, '', $expiry_date, 'text', []) . '</td>';
		$row .= '<td class="taxrate">' . $this->get_taxes_dropdown_template($name_tax_id_select, $invoice_item_taxes, 'invoice', $item_key, true, $manual) . '</td>';
	...
	public function packing_list_get_delivery_note($delivery_id) {
		...
		if(count($goods_delivery_details) > 0){
			...
			$base_currency_name = get_currency_name($currency);
    		$delivery_currency_name = get_currency_name($get_goods_delivery->currency);
    		$currency_exchange_rate_temp = wh_get_currency_rate($delivery_currency_name, $base_currency_name);
    		if(get_option('goods_receipt_do_not_convert_to_base_currency') == 0){
    			if((int)$get_goods_delivery->currency != (int)$currency){
					$currency_rate = (float)$currency_exchange_rate_temp;
    			}
			}
			if(get_option('goods_receipt_do_not_convert_to_base_currency') == 1){
				$currency = (float)$get_goods_delivery->currency;
				$currency_exchange_rate = (float)$currency_exchange_rate_temp;
			}
			...
			foreach ($goods_delivery_details as $key => $value) {
				...
				$unit_price = ((float)$value['unit_price'] * $currency_rate);
				...
				if($quantities > 0){
					...
					$total_goods_money = (float)$quantities*(float)$unit_price;
					...
					$sub_total = (float)$unit_price * (float)$quantities;
					$expiry_date ='';
					if(($value['expiry_date'] != null) && ( $value['expiry_date'] != '') ){
						$array_expiry_date = new_explode(',', $value['expiry_date']);
    ...
    public function add_packing_list($data, $id = false) {
		...
		if (isset($data['custom_fields'])) {
            $custom_fields = $data['custom_fields'];
            foreach ($custom_fields as $key => $value) {
                if(preg_match('/^iv_packing_list/', $key)){
                    $inventory_packing_list_custom_fields = [];
                    $inventory_packing_list_custom_fields[$key] = $value;
                }
            }
            unset($data['custom_fields']);
        }
		...
		if (isset($insert_id)) {
			...
			if (isset($inventory_packing_list_custom_fields)) {
                handle_custom_fields_post($insert_id, $inventory_packing_list_custom_fields);
            }
		...
		if (isset($insert_id)) {
			...
			hooks()->do_action('after_wh_packing_list_added', $insert_id);
		}
	...
	public function update_packing_list($data, $id = false) {
		...
		if (isset($inventory_packing_list_custom_fields)) {
            if(handle_custom_fields_post($packing_list_id, $inventory_packing_list_custom_fields)) {
                $results++;
            }
        }
	...
	public function delete_packing_list($id) {
		...
		// Delete the custom field values
        $this->db->where('relid', $id);
        $this->db->where('fieldto', 'iv_packing_list');
        $this->db->delete(db_prefix() . 'customfieldsvalues');
        if ($this->db->affected_rows() > 0) {
            $affected_rows++;
        }
	 ...
	 * @return [type]     
	 */
	public function get_html_tax_packing_list($id, $currency = 0)
	{
		$html = '';
		$html_currency = '';
		$preview_html = '';
	...
	public function delivery_status_mark_as($status, $id, $type)
	{
		...
		if($type == 'delivery'){
			...
			if ($this->db->affected_rows() > 0) {
				...
				hooks()->do_action('after_wh_packing_list_change_delivery_status', $id);
			}
		}
	 return $status_f;
	}
	 ...
	 * @return [type]     
	 */
	public function get_html_tax_order_return($id, $currency)
	{
		$html = '';
		$html_currency = '';
		$preview_html = '';
	...
	public function purchasing_return_order_create_stock_export($order_return_id, $data_item_warehouse)
	{
		...
		$data['description'] =  '';
		$currency_exchange_rate = (float)$order_return->currency_exchange_rate;
		...
		foreach ($order_return_details as $key => $order_return_detail) {
			...
			$note = '';
			$quantities = $order_return_detail['quantity'];
			$unit_price = (float)$order_return_detail['unit_price'] / $currency_exchange_rate;
			$lot_number = '';
			$date_manufacture = '';
			$expiry_date = '';
			$commodity_code = $order_return_detail['commodity_code'];
			...
			$newitems[] = [
				...
				'commodity_code' => $commodity_code,
				'unit_id' => $unit_id,
				'discount_money' => $order_return_detail['discount_total'] / $currency_exchange_rate,
				'total_after_discount' => $order_return_detail['total_after_discount'] / $currency_exchange_rate,
			];
		}

		$data['newitems'] = $newitems;
		$data['sub_total'] =  $order_return->subtotal / $currency_exchange_rate;
		$data['total_money'] =  $order_return->total_amount / $currency_exchange_rate;
		$data['total_discount'] =  $order_return->discount_total / $currency_exchange_rate;
		$data['after_discount'] =  $order_return->total_after_discount / $currency_exchange_rate;
		$data['additional_discount'] =  $order_return->additional_discount / $currency_exchange_rate;
			...
			if($data['receipt_delivery_type'] == 'inventory_receipt_voucher_returned_goods'){
				...
				//from inventory, Inventory receipt note
					...
					if($goods_receipt){
						...
						$return_reason = _l('wh_add_inventory_receipt_voucher_returned_goods');
						if($goods_receipt->currency != 0){
							$currency = $goods_receipt->currency;
							$currency_exchange_rate = $goods_receipt->currency_exchange_rate;
						}
					}
				...
				//get list vendor 
					...
					// check get PO and compare with base currency
						$this->load->model('purchase/purchase_model');
						$get_pur_order = $this->purchase_model->get_pur_order($order_return->rel_id);

						if($get_pur_order->currency != 0){
							$currency = $get_pur_order->currency;
							$currency_exchange_rate = $get_pur_order->currency_rate;
						}
	 ...
	 * @return [type]              
	 */
	public function getStaffAssignedToWarehouseHtml($warehouseId='0', $return_staff_ids = false, $staff_id = false)
	{
		$arrStaffIds = [];
		$staff_infors = [];
		$staff_ids = [];
		if($staff_id){
			$arrStaffIds[] = $staff_id;
		}
	...
	public function get_stock_balance_data($data)
	{
		...
		$this->db->where(db_prefix().'items.id  > 0');
	 ...
	 * @return [type]       
	 */
	public function get_approve_detail_wh($data, $action = ''){
        $this->db->where('rel_id', $data['rel_id']);
        $this->db->where('rel_type', $data['rel_type']);
        ...
        $details = $this->db->get(db_prefix().'wh_approval_details')->row();
        if($details){
            return $details;
        }
     ...
     * @return [type]     
     */
    public function get_item_image($id, $image_attr ='')
    {
    	$item_image = '';
    	if(!is_numeric($id)){
    		return $item_image;
    	}
    	$item_images = $this->item_attachments($id);
    	if (isset($item_images[$id]) && isset($item_images[$id][0])) {

    		if (file_exists(WAREHOUSE_ITEM_UPLOAD . $item_images[$id][0]['rel_id'] . '/' . $item_images[$id][0]['file_name'])) {
    			$item_image = '<img class="article_w_table" '.$image_attr.' src="' . site_url('modules/warehouse/uploads/item_img/' . $item_images[$id][0]['rel_id'] . '/' . $item_images[$id][0]['file_name']) . '" alt="' . $item_images[$id][0]['file_name'] . '" >';
    		} elseif(file_exists('modules/purchase/uploads/item_img/' . $item_images[$id][0]['rel_id'] . '/' . $item_images[$id][0]['file_name'])) {
    			$item_image = '<img class="article_w_table" '.$image_attr.' src="' . site_url('modules/purchase/uploads/item_img/' . $item_images[$id][0]['rel_id'] . '/' . $item_images[$id][0]['file_name']) . '" alt="' . $item_images[$id][0]['file_name'] . '" >';
    		}elseif(file_exists('modules/manufacturing/uploads/products/' . $item_images[$id][0]['rel_id'] . '/' . $item_images[$id][0]['file_name'])) {
    			$item_image = '<img class="article_w_table" '.$image_attr.' src="' . site_url('modules/manufacturing/uploads/products/' . $item_images[$id][0]['rel_id'] . '/' . $item_images[$id][0]['file_name']) . '" alt="' . $item_images[$id][0]['file_name'] . '" >';
    		}elseif(file_exists('modules/rental/uploads/products/' . $item_images[$id][0]['rel_id'] . '/' . $item_images[$id][0]['file_name'])) {
    			$item_image = '<img class="article_w_table" '.$image_attr.' src="' . site_url('modules/rental/uploads/products/' . $item_images[$id][0]['rel_id'] . '/' . $item_images[$id][0]['file_name']) . '" alt="' . $item_images[$id][0]['file_name'] . '" >';
			}else{
    			if (get_status_modules_wh('purchase')) {
    				$this->load->model('purchase/purchase_model');
    				$get_commodity = $this->get_commodity($id);
    				if(isset($get_commodity->from_vendor_item) && is_numeric($get_commodity->from_vendor_item)){
    					$vendor_image = $this->purchase_model->get_vendor_item_file($get_commodity->from_vendor_item);
    					if(count($vendor_image) > 0){
    						if(file_exists(PURCHASE_PATH.'vendor_items/' .$get_commodity->from_vendor_item .'/'.$vendor_image[0]['file_name'])){
    							$item_image = '<img class="article_w_table" '.$image_attr.' src="' . site_url('modules/purchase/uploads/vendor_items/' . $vendor_image[0]['rel_id'] .'/'.$vendor_image[0]['file_name']).'" alt="'.$vendor_image[0]['file_name'] .'" >';
    						}else{
    							$item_image = '<img class="article_w_table" '.$image_attr.' src="' . site_url('modules/warehouse/uploads/nul_image.jpg') . '" alt="nul_image.jpg">';
    						}
    					}
    				}else{
    					$item_image = '<img class="article_w_table" '.$image_attr.' src="' . site_url('modules/warehouse/uploads/nul_image.jpg') . '" alt="nul_image.jpg">';
    ...
    /**
     * get loss adjustment type
     * @param  array  $ids 
     * @return [type]      
     */
    public function get_loss_adjustment_type($ids = []) {
    	$arr_loss_adjustment = [];
    	if(count($ids) > 0){

    		$this->db->select('id, type');
    		$this->db->where('id IN ('.implode(',', $ids).')');
    		$loss_adjustment = $this->db->get(db_prefix() . 'wh_loss_adjustment')->result_array();
    		foreach ($loss_adjustment as $key => $value) {
    			$arr_loss_adjustment[$value['id']] = $value['type'];
    		}
    	}
    	return $arr_loss_adjustment;
    }
    ...
    {   
        ...
        if(isset($string1[1])){
            ...
            if(isset($rate[0])){
            	$rate[0]             = reformat_currency_j($rate[0]);
    ...
    public function get_currency_rate_infor($currency_id)
    {
        ...
        if($base_currency->id != $pr_currency->id){
            $currency_rate = wh_get_currency_rate($base_currency->name, $pr_currency->name);
	...
	/**
	 * print custom barcode pdf
	 * @param mixed $print_barcode
	 * @return object
	 */
	public function print_custom_barcode_pdf($print_barcode)
	{
		return app_pdf('print_barcode', module_dir_path(WAREHOUSE_MODULE_NAME, 'libraries/pdf/Print_custom_barcode_pdf.php'), $print_barcode);
	}

...
if(!class_exists('ClickatellException')){
    require (FCPATH.'modules/purchase/helpers/ClickatellException.php');
}
...
{   
     ...
     */
    public function get_contract($id = '',  $for_editor = false){
        if($id == ''){
    ...
    public function get_merge_fields(object $contract): array
    {
        ...
        $this->load->library('merge_fields/purchase_contract_merge_fields');
        ...
        $merge_fields = [];
        $merge_fields = array_merge($merge_fields, $this->purchase_contract_merge_fields->format($contract));
        $merge_fields = array_merge($merge_fields, $this->other_merge_fields->format());


       
        $this->load->library('merge_fields/vendor_merge_fields');
        $merge_fields = array_merge($merge_fields, $this->vendor_merge_fields->format( '', $contract->vendor, '')); 
    ...
    $html = '<table class="table">
      ...
      $html .=  '<table class="table pur_request-item">
            <thead>
              <tr class="border_tr">
              <th align="left" class=" thead-dark">#</th>
                <th align="left" class="width15 thead-dark">'._l('items').'</th>
      ...
      foreach($pur_request_detail as $row){
        ...
        if($items){
            ...
            $arr_images = $this->purchase_model->get_item_attachments($items->id);

            if(count($arr_images) > 0){
               if(file_exists(PURCHASE_MODULE_ITEM_UPLOAD_FOLDER .$arr_images[0]['rel_id'] .'/'.$arr_images[0]['file_name'])){
                   $img = '<td><img class="images_w_table" src="' . APP_MODULES_PATH.'purchase/uploads/item_img/' . $arr_images[0]['rel_id'] .'/'.$arr_images[0]['file_name'].'" alt="'.$arr_images[0]['file_name'] .'" ></td>';
               }else if(file_exists('modules/warehouse/uploads/item_img/' . $arr_images[0]['rel_id'] . '/' . $arr_images[0]['file_name'])){
                   $img = '<td><img class="images_w_table" src="' . APP_MODULES_PATH.'warehouse/uploads/item_img/' . $arr_images[0]['rel_id'] .'/'.$arr_images[0]['file_name'].'" alt="'.$arr_images[0]['file_name'] .'" ></td>';
               }else if(file_exists('modules/manufacturing/uploads/products/' . $arr_images[0]['rel_id'] . '/' . $arr_images[0]['file_name'])) {
                   $img = '<td><img class="images_w_table" src="' . APP_MODULES_PATH.'manufacturing/uploads/products/' . $arr_images[0]['rel_id'] . '/' . $arr_images[0]['file_name'] . '" alt="' . $arr_images[0]['file_name'] . '" ></td>';
               }else{
                   
                   $img = '<td><img class="images_w_table" src="' . APP_MODULES_PATH.'purchase/uploads/nul_image.jpg" alt="nul_image.jpg"></td>'; 
            ...
            $html .= '<tr class="border_tr">
                '.$img.'
                <td class="width15" align="left">'.$items->commodity_code.' - '.$items->description.'</td>
            ...
            $unit_name = isset($units->unit_name) ? $units->unit_name : '';
            $html .= '<tr class="border_tr">
                <td><img class="images_w_table" src="' . APP_MODULES_PATH .'purchase/uploads/nul_image.jpg" alt="nul_image.jpg"></td>
    ...
    $html = '<table class="table">
      ...
      $html .=  '<table class="table pur_request-item">
            <thead>
              <tr class="border_tr">
               <th align="left" class="thead-dark">#</th>
                <th align="left" class="thead-dark width15">'._l('items').'</th>
    ...
    {   
        ...
        if(isset($data['pur_employees_receive_notifications_when_new_vendor_registers'])){

            $data['pur_employees_receive_notifications_when_new_vendor_registers'] = implode(',', $data['pur_employees_receive_notifications_when_new_vendor_registers']);

            if( update_option('pur_employees_receive_notifications_when_new_vendor_registers', $data['pur_employees_receive_notifications_when_new_vendor_registers'])){
                $rs++;
            }
        }
...
/**
    ...
    */
    ...
    public function add_debit_note($data){
        ...
        if ($insert_id) {
            ...
            hooks()->do_action('after_pur_debit_note_added', $insert_id);
    ...
    public function update_debit_note($data, $id)
    {
        ...
        if ($affectedRows > 0) {
            hooks()->do_action('after_pur_debit_note_updated', $id);
    ...
    */
    public function delete_debit_note($id, $simpleDelete = false)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            hooks()->do_action('after_pur_debit_note_deleted', $insert_id);
    ...
    public function apply_debits($id, $data)
    {
        ...
        if ($insert_id) {
            ...
            hooks()->do_action('after_pur_debits_applied', $insert_id);
        }
    ...
    public function delete_applied_debit($id, $debit_id, $invoice_id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            hooks()->do_action('after_pur_applied_debit_deleted', $id);
        }
    }
    ...
    public function mark_debit_note($id, $status)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            hooks()->do_action('after_pur_debit_note_status_changed', $id, ['status' => $status]);
    ...
    public function create_refund($id, $data)
    {
        ...
        if ($insert_id) {
            ...
            hooks()->do_action('after_pur_debit_note_refund_created', $insert_id);
        }
    ...
    public function edit_refund($id, $data)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            hooks()->do_action('after_pur_debit_note_refund_updated', $id);
        }
    ...
    public function delete_refund($refund_id, $debit_note_id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            hooks()->do_action('after_pur_debit_note_refund_deleted', $refund_id);
            return true;
        }
  ...
  /**
    ...
    /**
     * [send_notifi_new_vendor_for_staff description]
     * @return [type] [description]
     */
    public function send_notifi_new_vendor_for_staff($vendor_id){

        $primary_contact = $this->get_primary_contacts($vendor_id);

        if(isset($primary_contact->id)){

            $staff_ids = get_option('pur_employees_receive_notifications_when_new_vendor_registers');
            ...
            if(count($staff_ids_arr) > 0){
                $this->load->model('staff_model');
                foreach($staff_ids_arr as $staff_id){


                    $staff = $this->staff_model->get($staff_id);
                    if(isset($staff->staffid)){
                        $additional_data = get_vendor_company_name($vendor_id);

                        $notified = add_notification([
                        'description'     => _l('new_vendor_registration').' '. $additional_data,
                        'touserid'        => $staff->staffid,
                        'link'            => 'purchase/vendor/'.$vendor_id,
                        'additional_data' => serialize([
                            $additional_data,
                        ]),
                        ]);
                        ...
                        if(get_option('sms_notification_when_new_vendor_registers') == 1){
                            if($staff->phonenumber != ''){
                                $message = _l('new_vendor_registration').' '. $additional_data;

                                $success = $this->sendSMS($message, $staff->phonenumber);
                        ...
                        $data_sm = [];
                        $data_sm['staff'] = $staff;
                        $data_sm['contact'] = $primary_contact;

                       
                        $template = mail_template('new_vendor_register', 'purchase', array_to_object($data_sm));
                        $template->send();
                    }
                }
            }

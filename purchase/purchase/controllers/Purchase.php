...
class purchase extends AdminController
{
    ...
    public function vendor($id = '')
    {
        ...
        if ($id == '') {
            ...
            $data['tab'] = hooks()->apply_filters('purchase_vendor_profile_tabs', $data['tab'], $client);
	        
	        if($data['group'] == ''){
	            $data['group'] = 'profile';
	        }
            $extension_groups = hooks()->apply_filters('purchase_vendor_profile_extension_groups', [], $client);
	        $data['tabs']['view'] = in_array($data['group'], $extension_groups, true)
                ? 'vendors/groups/profile_extension'
                : 'vendors/groups/'.$data['group'];
            if ($data['tabs']['view'] !== 'vendors/groups/profile_extension'
                && !file_exists(module_views_path('purchase', $data['tabs']['view'] . '.php'))) {
                $data['group'] = 'profile';
                $data['tabs']['view'] = 'vendors/groups/profile';
            }
    ...
    public function purchase_order($id = ''){
        ...
        if ($this->db->table_exists(db_prefix() . 'pur_orders')) {
          if (!$this->db->field_exists('acc_mapping' ,db_prefix() . 'pur_orders')) {
            $this->db->query("ALTER TABLE `" . db_prefix() . "pur_orders`
        ...
        if ($this->db->table_exists(db_prefix() . 'pur_invoices')) {
          if (!$this->db->field_exists('acc_mapping' ,db_prefix() . 'pur_invoices')) {
            $this->db->query("ALTER TABLE `" . db_prefix() . "pur_invoices`
              ADD COLUMN `acc_mapping` tinyint(1) NOT NULL DEFAULT '0'
              ");
          }
        }
        ...
        if ($this->db->table_exists(db_prefix() . 'pur_invoice_payment')) {
          if (!$this->db->field_exists('acc_mapping' ,db_prefix() . 'pur_invoice_payment')) {
            $this->db->query("ALTER TABLE `" . db_prefix() . "pur_invoice_payment`
              ADD COLUMN `acc_mapping` tinyint(1) NOT NULL DEFAULT '0'
              ");
          }
        }

        if ($this->db->table_exists(db_prefix() . 'wh_order_returns_refunds')) {
          if (!$this->db->field_exists('acc_mapping' ,db_prefix() . 'wh_order_returns_refunds')) {
            $this->db->query("ALTER TABLE `" . db_prefix() . "wh_order_returns_refunds`
              ADD COLUMN `acc_mapping` tinyint(1) NOT NULL DEFAULT '0'
              ");
          }
        }
    ...
    public function contract($id = ''){
        ...
        if ($id == '') {
            ...
            $data['contract'] = $this->purchase_model->get_contract($id, true);
        ...
        $data['contract_merge_fields'] = $this->app_merge_fields->get_flat('purchase_contract', ['other', 'vendor'], '{email_signature}');
...
/**
    ...
    public function purchase_inv_report()
    {
     ...
     /**
     *  purchase inv payment report
     *  
     *  @return json
     */
    public function purchase_inv_payment_report()
    {
        if ($this->input->is_ajax_request()) {
            $this->load->model('currencies_model');
            ...
            $select = [
                ...
                'invoice_number',
                'paymentmode',
                db_prefix().'pur_invoice_payment.transactionid as transactionid',
                'date',
                db_prefix().'pur_invoice_payment.approval_status as approve_status',
                'amount',
       
            ];
            $where =[];
            $custom_date_select = $this->get_where_report_period(db_prefix() . 'pur_invoice_payment.date');
            if ($custom_date_select != '') {
                array_push($where, $custom_date_select);
            }
            ...
            if($this->input->post('approve_status') && $this->input->post('approve_status') != ''){
                array_push($where, 'AND '.db_prefix().'pur_invoice_payment.approval_status = '.$this->input->post('approve_status'));
            }

            if($this->input->post('payment_mode') && $this->input->post('payment_mode') != ''){
                array_push($where, 'AND '.db_prefix().'pur_invoice_payment.paymentmode = "'.$this->input->post('payment_mode').'"');
            }
            ...
            $join         = [
                'LEFT JOIN '.db_prefix().'pur_invoices ON '.db_prefix().'pur_invoice_payment.pur_invoice = '.db_prefix().'pur_invoices.id'
            ];
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = '<a href="' . admin_url('purchase/purchase_invoice/' . $aRow['pur_invoice']) . '" target="_blank">' . $aRow['invoice_number'] . '</a>';

                $row[] = get_payment_mode_by_id($aRow['paymentmode']);
    ...
    public function contract_change($ct){
        $contract = $this->purchase_model->get_contract($ct, true);
        $value = 0;
        if($contract){

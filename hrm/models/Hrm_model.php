...
class hrm_model extends App_Model
{
    ...
    public function get_department_name($departmentid)
    {
        ...
        $this->db->where('departmentid', (int) $departmentid);
    ...
    public function add_department($data)
    {
        ...
        if ($this->db->field_exists('manager_id', db_prefix() . 'departments')) {
            $insert['manager_id'] = isset($data['manager_id']) ? (int) $data['manager_id'] : 0;
        }
        $this->db->insert(db_prefix() . 'departments', $insert);
    ...
    public function update_department($data, $id)
    {
        ...
        if ($this->db->field_exists('manager_id', db_prefix() . 'departments') && isset($data['manager_id'])) {
            $update['manager_id'] = (int) $data['manager_id'];
        }
        $this->db->where('departmentid', $id);
        $this->db->update(db_prefix() . 'departments', $update);
        return $this->db->affected_rows() > 0;
    }

    public function delete_department($id)
    {
        $refs = $this->db->where('departmentid', $id)->count_all_results(db_prefix() . 'staff_departments');
        if ($refs > 0) {
   ...
   return $result;
    ...
    public function contract_type_chart()
    {
        ...
        foreach ($statuses as $key => $status) {
			  ...
			  if ( $total_value == null ) { $total_value = '1'; }
          }
    ...
    /**
     * Staff ratio by departments for dashboard chart
     */
    public function staff_chart_by_department()
    {
        ...
        foreach ($departments as $dept) {
            $dept_counts[$dept['departmentid']] = ['name' => $dept['name'], 'count' => 0];
        }
        $dept_counts[0] = ['name' => _l('no_department'), 'count' => 0];
        foreach ($staffs as $staff) {
            $staff_id = is_object($staff) ? $staff->staffid : ($staff['staffid'] ?? 0);
            $staff_depts = $this->departments_model->get_staff_departments($staff_id);
            if (!empty($staff_depts)) {
                $first_dept = $staff_depts[0]['departmentid'] ?? 0;
                if (isset($dept_counts[$first_dept])) {
                    $dept_counts[$first_dept]['count']++;
                } else {
                    $dept_counts[0]['count']++;
                }
            } else {
        ...
        $total = count($staffs);
        $i = 0;
        foreach ($dept_counts as $dept_id => $data) {
            if ($data['count'] > 0) {
                $_data[] = [
                    'name' => $data['name'],
                    'y' => (int) $data['count'],
                    'z' => $total > 0 ? round(($data['count'] / $total) * 100, 2) : 0,
    ...
    /**
     * Staff ratio by jobs (positions) for dashboard chart
     */
    public function staff_chart_by_jobs()
    {
        $staffs = $this->staff_model->get();
        $positions = $this->get_job_position();
        $color_data = ['#00FF7F', '#0cffe95c', '#80da22', '#f37b15', '#da1818', '#176cea', '#5be4f0', '#57c4d8', '#a4d17a'];
        $_data = [];
        $pos_counts = [];
        foreach ($positions as $pos) {
            $pos_counts[$pos['position_id']] = ['name' => $pos['position_name'], 'count' => 0];
        }
        $pos_counts[0] = ['name' => _l('no_position'), 'count' => 0];
        foreach ($staffs as $staff) {
            $pos_id = is_object($staff) ? ($staff->job_position ?? 0) : ($staff['job_position'] ?? 0);
            if (isset($pos_counts[$pos_id])) {
                $pos_counts[$pos_id]['count']++;
            } else {
                $pos_counts[0]['count']++;
            }
        }
        $total = count($staffs);
        $i = 0;
        foreach ($pos_counts as $pos_id => $data) {
            if ($data['count'] > 0) {
                $_data[] = [
                    'name' => $data['name'],
                    'y' => (int) $data['count'],
    ...
    /**
     * Ratio of changes in staff by seniority (hired vs left by tenure)
     */
    public function staff_changes_by_seniority()
    {
        $hired_this_year = $this->db->query("SELECT s.staffid, s.datecreated FROM " . db_prefix() . "staff s WHERE YEAR(s.datecreated) = " . date('Y'))->result_array();
        $left_this_year = $this->db->query("SELECT s.staffid, s.date_update FROM " . db_prefix() . "staff s WHERE s.status_work = 'inactivity' AND YEAR(s.date_update) = " . date('Y'))->result_array();
        $buckets = ['0-1' => ['hired' => 0, 'left' => 0], '1-3' => ['hired' => 0, 'left' => 0], '3-5' => ['hired' => 0, 'left' => 0], '5+' => ['hired' => 0, 'left' => 0]];
        foreach ($hired_this_year as $s) {
            $buckets['0-1']['hired']++;
        }
        foreach ($left_this_year as $s) {
            $staff = $this->staff_model->get($s['staffid']);
            if ($staff) {
                $start = strtotime($staff->datecreated ?? $s['date_update']);
                $end = strtotime($s['date_update'] ?? date('Y-m-d'));
                $years = ($end - $start) / (365 * 24 * 3600);
                if ($years < 1) $buckets['0-1']['left']++;
                elseif ($years < 3) $buckets['1-3']['left']++;
                elseif ($years < 5) $buckets['3-5']['left']++;
                else $buckets['5+']['left']++;
            }
        }
        return $buckets;
    ...
    /**
     * Staff status by months (trend - last 12 months)
     */
    public function staff_status_by_months()
    {
        $months = [];
        for ($i = 11; $i >= 0; $i--) {
            ...
            $hired = $this->db->query("SELECT COUNT(*) as c FROM " . db_prefix() . "staff WHERE MONTH(datecreated) = $m AND YEAR(datecreated) = $y")->row()->c;
            $left = $this->db->query("SELECT COUNT(*) as c FROM " . db_prefix() . "staff WHERE status_work = 'inactivity' AND MONTH(date_update) = $m AND YEAR(date_update) = $y")->row()->c;
            $months[] = ['label' => $label, 'hired' => (int) $hired, 'left' => (int) $left];
        }
        return $months;
    }
    ...
    /**
     * Get list of payslips (latched payroll tables) available for a staff member
     */
    public function get_payslip_list_for_staff($staff_id)
    {
        $this->db->select('pt.id, pt.payroll_month, pt.payroll_type, pty.payroll_type_name');
        $this->db->from(db_prefix() . 'payroll_table pt');
        $this->db->join(db_prefix() . 'payroll_type pty', 'pty.id = pt.payroll_type', 'left');
        $this->db->where('pt.status', 1);
        $this->db->order_by('pt.payroll_month', 'desc');
        return $this->db->get()->result_array();
    }
    ...
    public function get_layoff_checklist()
    {
        $this->db->order_by('sort_order', 'asc');
        return $this->db->get(db_prefix() . 'hrm_layoff_checklist')->result_array();
    }

    public function add_layoff_checklist_item($data)
    {
        $this->db->insert(db_prefix() . 'hrm_layoff_checklist', $data);
        return $this->db->insert_id();
    }

    public function update_layoff_checklist_item($data, $id)
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'hrm_layoff_checklist', $data);
        return $this->db->affected_rows() > 0;
    }

    public function delete_layoff_checklist_item($id)
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'hrm_layoff_checklist');
        return $this->db->affected_rows() > 0;
    }

    public function get_training_types()
    {
        return $this->db->get(db_prefix() . 'hrm_training_types')->result_array();
    }

    public function add_training_type($data)
    {
        $this->db->insert(db_prefix() . 'hrm_training_types', $data);
        return $this->db->insert_id();
    }

    public function update_training_type($data, $id)
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'hrm_training_types', $data);
        return $this->db->affected_rows() > 0;
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'hrm_training_types');
        return $this->db->affected_rows() > 0;
    }

    public function get_onboarding_templates()
    {
        return $this->db->get(db_prefix() . 'hrm_onboarding_templates')->result_array();
    }

    public function add_onboarding_template($data)
    {
        if (isset($data['checklist_items']) && is_array($data['checklist_items'])) {
            $data['checklist_items'] = json_encode($data['checklist_items']);
        }
        $this->db->insert(db_prefix() . 'hrm_onboarding_templates', $data);
        return $this->db->insert_id();
    }

    public function update_onboarding_template($data, $id)
    {
        if (isset($data['checklist_items']) && is_array($data['checklist_items'])) {
            $data['checklist_items'] = json_encode($data['checklist_items']);
        }
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'hrm_onboarding_templates', $data);
        return $this->db->affected_rows() > 0;
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'hrm_onboarding_templates');
        return $this->db->affected_rows() > 0;
    }

    public function get_onboarding_records($staff_id = null)
    {
        $this->db->select('r.*, t.name as template_name, t.checklist_items');
        $this->db->from(db_prefix() . 'hrm_onboarding_records r');
        $this->db->join(db_prefix() . 'hrm_onboarding_templates t', 't.id = r.template_id', 'left');
        if ($staff_id) $this->db->where('r.staff_id', $staff_id);
        $this->db->order_by('r.started_date', 'desc');
        return $this->db->get()->result_array();
    }

    public function add_onboarding_record($data)
    {
        $data['started_date'] = !empty($data['started_date']) ? to_sql_date($data['started_date']) : date('Y-m-d');
        ...
        $this->db->insert(db_prefix() . 'hrm_onboarding_records', $data);
        return $this->db->insert_id();
    }

    public function update_onboarding_record($data, $id)
    {
        if (isset($data['started_date']) && !empty($data['started_date'])) $data['started_date'] = to_sql_date($data['started_date']);
        if (isset($data['completed_date']) && !empty($data['completed_date'])) $data['completed_date'] = to_sql_date($data['completed_date']);
        if (isset($data['checklist_data']) && is_array($data['checklist_data'])) $data['checklist_data'] = json_encode($data['checklist_data']);
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'hrm_onboarding_records', $data);
        return $this->db->affected_rows() > 0;
    }

    public function get_contract_templates()
    {
        return $this->db->get(db_prefix() . 'hrm_contract_templates')->result_array();
    }

    public function get_contract_templates_by_type($contract_type_id)
    {
        $contract_type_id = (int) $contract_type_id;
        if ($contract_type_id <= 0) {
            return [];
        }
        $this->db->where('contract_type_id', $contract_type_id);
        $this->db->order_by('name', 'asc');
        return $this->db->get(db_prefix() . 'hrm_contract_templates')->result_array();
    }

    public function get_contract_template($id)
    {
        return $this->db->get_where(db_prefix() . 'hrm_contract_templates', ['id' => (int) $id])->row_array();
    }

    public function add_contract_template($data)
    {
        $this->db->insert(db_prefix() . 'hrm_contract_templates', $data);
        return $this->db->insert_id();
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'hrm_contract_templates', $data);
        return $this->db->affected_rows() > 0;
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'hrm_contract_templates');
        return $this->db->affected_rows() > 0;
    }

    /**
     * Persist the template id + rendered HTML body on a staff_contract row.
     */
    public function update_contract_body($contract_id, $template_id, $body_html)
    {
        $this->db->where('id_contract', (int) $contract_id);
        $this->db->update(db_prefix() . 'staff_contract', [
            'template_id'  => $template_id !== '' && $template_id !== null ? (int) $template_id : null,
            'body_content' => (string) $body_html,
        ]);
        return $this->db->affected_rows() > 0;
    }

    /**
     * Store an electronic signature (base64 PNG) on a contract.
     */
    public function update_contract_signature($contract_id, $signature_image, $ip = null)
    {
        $this->db->where('id_contract', (int) $contract_id);
        $this->db->update(db_prefix() . 'staff_contract', [
            'signature_image' => (string) $signature_image,
            'signed_at'       => date('Y-m-d H:i:s'),
            'signed_ip'       => $ip ? substr((string) $ip, 0, 45) : null,
        ]);
        return $this->db->affected_rows() > 0;
    }

    public function get_dependants($staff_id = null)
    {
        if ($staff_id) {
            $this->db->where('staff_id', $staff_id);
        }
        return $this->db->get(db_prefix() . 'hrm_dependants')->result_array();
    }

    public function add_dependant($data)
    {
        $data['date_of_birth'] = !empty($data['date_of_birth']) ? to_sql_date($data['date_of_birth']) : null;
        $this->db->insert(db_prefix() . 'hrm_dependants', $data);
        return $this->db->insert_id();
    }

    public function update_dependant($data, $id)
    {
        if (isset($data['date_of_birth']) && !empty($data['date_of_birth'])) {
            $data['date_of_birth'] = to_sql_date($data['date_of_birth']);
        }
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'hrm_dependants', $data);
        return $this->db->affected_rows() > 0;
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'hrm_dependants');
        return $this->db->affected_rows() > 0;
    }

    public function get_staff_trainings($staff_id = null)
    {
        $this->db->select('t.*, tt.name as training_type_name');
        $this->db->from(db_prefix() . 'hrm_staff_trainings t');
        $this->db->join(db_prefix() . 'hrm_training_types tt', 'tt.id = t.training_type_id', 'left');
        if ($staff_id) {
    ...
    public function add_staff_training($data)
    {
        $data['completed_date'] = !empty($data['completed_date']) ? to_sql_date($data['completed_date']) : null;
        $this->db->insert(db_prefix() . 'hrm_staff_trainings', $data);
        return $this->db->insert_id();
    }

    public function update_staff_training($data, $id)
    {
        if (isset($data['completed_date']) && !empty($data['completed_date'])) {
            $data['completed_date'] = to_sql_date($data['completed_date']);
        }
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'hrm_staff_trainings', $data);
        return $this->db->affected_rows() > 0;
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'hrm_staff_trainings');
        return $this->db->affected_rows() > 0;
    }

    public function get_policies($category = null, $is_faq = null)
    {
        if ($category) $this->db->where('category', $category);
        if ($is_faq !== null) $this->db->where('is_faq', $is_faq);
        $this->db->order_by('sort_order', 'asc');
        return $this->db->get(db_prefix() . 'hrm_policies')->result_array();
    }
    ...
    public function add_policy($data)
    {
        ...
        $this->db->insert(db_prefix() . 'hrm_policies', $data);
        return $this->db->insert_id();
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'hrm_policies', $data);
        return $this->db->affected_rows() > 0;
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'hrm_policies');
        return $this->db->affected_rows() > 0;
    }

    public function get_layoff_records($staff_id = null)
    {
        if ($staff_id) $this->db->where('staff_id', $staff_id);
        $this->db->order_by('layoff_date', 'desc');
        return $this->db->get(db_prefix() . 'hrm_layoff_records')->result_array();
    }

    public function add_layoff_record($data)
    {
        $data['layoff_date'] = !empty($data['layoff_date']) ? to_sql_date($data['layoff_date']) : null;
        $data['date_added'] = date('Y-m-d H:i:s');
        $data['added_from'] = get_staff_user_id();
        if (isset($data['checklist_completed']) && is_array($data['checklist_completed'])) {
            $data['checklist_completed'] = json_encode($data['checklist_completed']);
        }
        $this->db->insert(db_prefix() . 'hrm_layoff_records', $data);
        return $this->db->insert_id();
    }

    public function get_job_description_groups()
    {
        return $this->db->get(db_prefix() . 'hrm_job_description_groups')->result_array();
    }

    public function add_job_description_group($data)
    {
        $this->db->insert(db_prefix() . 'hrm_job_description_groups', $data);
        return $this->db->insert_id();
    }

    public function update_job_description_group($data, $id)
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'hrm_job_description_groups', $data);
        return $this->db->affected_rows() > 0;
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'hrm_job_description_groups');
        return $this->db->affected_rows() > 0;
    }

    public function get_assets($staff_id = null)
    {
        if ($staff_id) {
            $this->db->where('assigned_to', $staff_id);
        }
        return $this->db->get(db_prefix() . 'hrm_assets')->result_array();
    }

    public function get_assets_by_staff($staff_id)
    {
        return $this->get_assets($staff_id);
    }

    public function add_asset($data)
    {
        $data['assigned_date'] = !empty($data['assigned_date']) ? to_sql_date($data['assigned_date']) : null;
        $this->db->insert(db_prefix() . 'hrm_assets', $data);
        return $this->db->insert_id();
    }

    public function update_asset($data, $id)
    {
        if (isset($data['assigned_date']) && !empty($data['assigned_date'])) {
            $data['assigned_date'] = to_sql_date($data['assigned_date']);
        }
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'hrm_assets', $data);
        return $this->db->affected_rows() > 0;
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'hrm_assets');
        return $this->db->affected_rows() > 0;
    }

    public function get_staff_projects($staff_id)
    {
        if (!$this->db->table_exists(db_prefix() . 'project_members')) {
            return [];
        }
        $this->db->select('p.id, p.name, p.status');
        $this->db->from(db_prefix() . 'projects p');
        $this->db->join(db_prefix() . 'project_members pm', 'pm.project_id = p.id');
        $this->db->where('pm.staff_id', $staff_id);
        return $this->db->get()->result_array();
    }

    public function get_report_salary_changes()
    {
        $staff_with_multiple_salary = $this->db->query("
            SELECT sc.staff, s.firstname, s.lastname, 
                   (SELECT COUNT(*) FROM " . db_prefix() . "staff_contract_detail WHERE staff_contract_id = sc.id_contract) as detail_count,
                   (SELECT MIN(since_date) FROM " . db_prefix() . "staff_contract_detail WHERE staff_contract_id = sc.id_contract) as first_date,
                   (SELECT MAX(since_date) FROM " . db_prefix() . "staff_contract_detail WHERE staff_contract_id = sc.id_contract) as last_date
            FROM " . db_prefix() . "staff_contract sc
            LEFT JOIN " . db_prefix() . "staff s ON s.staffid = sc.staff
            WHERE (SELECT COUNT(*) FROM " . db_prefix() . "staff_contract_detail WHERE staff_contract_id = sc.id_contract) > 1
        ")->result_array();
        return $staff_with_multiple_salary;
    }

    public function get_report_seniority_changes()
    {
        $hires = $this->db->query("
            SELECT YEAR(datecreated) as yr, MONTH(datecreated) as mo, COUNT(*) as cnt
            ...
            LIMIT 24
        ")->result_array();
        $layoffs = $this->db->query("
            SELECT YEAR(layoff_date) as yr, MONTH(layoff_date) as mo, COUNT(*) as cnt
            FROM " . db_prefix() . "hrm_layoff_records
            WHERE layoff_date IS NOT NULL
            GROUP BY YEAR(layoff_date), MONTH(layoff_date)
            ORDER BY yr DESC, mo DESC
            LIMIT 24
        ")->result_array();
        return ['hires' => $hires, 'layoffs' => $layoffs];
    }

    public function get_report_monthly_changes()
    {
        $data = $this->db->query("
            SELECT YEAR(datecreated) as yr, MONTH(datecreated) as mo, COUNT(*) as new_count
            FROM " . db_prefix() . "staff
            WHERE datecreated IS NOT NULL
            GROUP BY YEAR(datecreated), MONTH(datecreated)
            ORDER BY yr DESC, mo DESC
        ...
        ")->result_array();
        $layoffs = $this->db->query("
            SELECT YEAR(layoff_date) as yr, MONTH(layoff_date) as mo, COUNT(*) as leave_count
            FROM " . db_prefix() . "hrm_layoff_records
            WHERE layoff_date IS NOT NULL
            GROUP BY YEAR(layoff_date), MONTH(layoff_date)
        ")->result_array();
        $layoff_map = [];
        foreach ($layoffs as $l) {
            $layoff_map[$l['yr'] . '-' . $l['mo']] = $l['leave_count'];
        }
        foreach ($data as &$d) {
            $d['leave_count'] = $layoff_map[$d['yr'] . '-' . $d['mo']] ?? 0;
        }
        return $data;
    }

    public function get_report_qualifications()
    {
        $this->db->select('s.staffid, s.firstname, s.lastname, s.literacy, d.name as department_name');
        ...
        $training_counts = [];
        $trainings = $this->db->get(db_prefix() . 'hrm_staff_trainings')->result_array();
        foreach ($trainings as $t) {
            $training_counts[$t['staff_id']] = ($training_counts[$t['staff_id']] ?? 0) + 1;
        }
        foreach ($staff as &$s) {
            $s['training_count'] = $training_counts[$s['staffid']] ?? 0;
        }
        return $staff;
    }

    public function get_helpdesk_tickets($staff_id = null)
    {
        if ($staff_id) $this->db->where('staff_id', $staff_id);
        $this->db->order_by('date_added', 'desc');
        return $this->db->get(db_prefix() . 'hrm_helpdesk_tickets')->result_array();
    }

    public function add_helpdesk_ticket($data)
    {
        $data['date_added'] = date('Y-m-d H:i:s');
        $data['date_updated'] = date('Y-m-d H:i:s');
        $this->db->insert(db_prefix() . 'hrm_helpdesk_tickets', $data);
        return $this->db->insert_id();
    }

    public function update_helpdesk_ticket($data, $id)
    {
        $data['date_updated'] = date('Y-m-d H:i:s');
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'hrm_helpdesk_tickets', $data);
        return $this->db->affected_rows() > 0;
    }

    public function get_performance_reviews($staff_id = null)
    {
        if ($staff_id) $this->db->where('staff_id', $staff_id);
        $this->db->order_by('review_date', 'desc');
        return $this->db->get(db_prefix() . 'hrm_performance_reviews')->result_array();
    }

    public function add_performance_review($data)
    {
        $data['review_date'] = !empty($data['review_date']) ? to_sql_date($data['review_date']) : null;
        $data['date_added'] = date('Y-m-d H:i:s');
        $this->db->insert(db_prefix() . 'hrm_performance_reviews', $data);
        return $this->db->insert_id();
    }

    public function get_performance_goals($staff_id = null, $review_id = null)
    {
        if ($staff_id) $this->db->where('staff_id', $staff_id);
        if ($review_id) $this->db->where('review_id', $review_id);
        return $this->db->get(db_prefix() . 'hrm_performance_goals')->result_array();
    }

    public function add_performance_goal($data)
    {
        $data['target_date'] = !empty($data['target_date']) ? to_sql_date($data['target_date']) : null;
        $data['date_added'] = date('Y-m-d H:i:s');
        $this->db->insert(db_prefix() . 'hrm_performance_goals', $data);
        return $this->db->insert_id();
    }

    public function get_learning_courses()
    {
        return $this->db->get(db_prefix() . 'hrm_learning_courses')->result_array();
    }

    public function add_learning_course($data)
    {
        $data['date_added'] = date('Y-m-d H:i:s');
        $this->db->insert(db_prefix() . 'hrm_learning_courses', $data);
        return $this->db->insert_id();
    }

    public function get_staff_courses($staff_id = null)
    {
        $this->db->select('sc.*, lc.name as course_name');
        $this->db->from(db_prefix() . 'hrm_staff_courses sc');
        $this->db->join(db_prefix() . 'hrm_learning_courses lc', 'lc.id = sc.course_id');
        if ($staff_id) $this->db->where('sc.staff_id', $staff_id);
        return $this->db->get()->result_array();
    }

    public function add_staff_course($data)
    {
        $data['completed_date'] = !empty($data['completed_date']) ? to_sql_date($data['completed_date']) : null;
        $data['date_added'] = date('Y-m-d H:i:s');
        $this->db->insert(db_prefix() . 'hrm_staff_courses', $data);
        return $this->db->insert_id();
    }

    public function get_engagement_surveys()
    {
        return $this->db->get(db_prefix() . 'hrm_engagement_surveys')->result_array();
    }

    public function add_engagement_survey($data)
    {
        $data['date_from'] = !empty($data['date_from']) ? to_sql_date($data['date_from']) : null;
        $data['date_to'] = !empty($data['date_to']) ? to_sql_date($data['date_to']) : null;
        $data['date_added'] = date('Y-m-d H:i:s');
        $data['added_by'] = get_staff_user_id();
        $this->db->insert(db_prefix() . 'hrm_engagement_surveys', $data);
        return $this->db->insert_id();
    }

    public function get_one_on_one_notes($staff_id = null)
    {
        if ($staff_id) $this->db->where('staff_id', $staff_id);
        $this->db->order_by('meeting_date', 'desc');
        return $this->db->get(db_prefix() . 'hrm_one_on_one_notes')->result_array();
    }

    public function add_one_on_one_note($data)
    {
        $data['meeting_date'] = !empty($data['meeting_date']) ? to_sql_date($data['meeting_date']) : null;
        $data['date_added'] = date('Y-m-d H:i:s');
        $this->db->insert(db_prefix() . 'hrm_one_on_one_notes', $data);
        return $this->db->insert_id();
    }

    public function get_hr_documents($category = null)
    {
        if ($category) $this->db->where('category', $category);
        return $this->db->get(db_prefix() . 'hrm_documents')->result_array();
    }

    public function add_hr_document($data)
    {
        $data['date_added'] = date('Y-m-d H:i:s');
        $data['added_by'] = get_staff_user_id();
        $this->db->insert(db_prefix() . 'hrm_documents', $data);
        return $this->db->insert_id();
    }
     ...
     * ------------------------------------------------------------------- */
    public function get_insurance_category($id = false)
    {
        if (is_numeric($id)) {
            return $this->db->get_where(db_prefix() . 'hrm_insurance_category', ['id' => (int) $id])->row_array();
        }
        return $this->db->get(db_prefix() . 'hrm_insurance_category')->result_array();
    }

    public function add_insurance_category($data)
    {
        $this->db->insert(db_prefix() . 'hrm_insurance_category', $data);
        return $this->db->insert_id();
    }
    ...
    public function update_insurance_category($data, $id)
    {
        ...
        $this->db->update(db_prefix() . 'hrm_insurance_category', $data);
        return $this->db->affected_rows() > 0;
    }
    ...
    {
        $this->db->where('id', (int) $id);
        $this->db->delete(db_prefix() . 'hrm_insurance_category');
        return $this->db->affected_rows() > 0;
    }
     ...
     * ------------------------------------------------------------------- */
    public function get_deduction_type($id = false)
    {
        if (is_numeric($id)) {
            return $this->db->get_where(db_prefix() . 'hrm_deduction_type', ['id' => (int) $id])->row_array();
        }
        return $this->db->get(db_prefix() . 'hrm_deduction_type')->result_array();
    }

    public function add_deduction_type($data)
    {
        $this->db->insert(db_prefix() . 'hrm_deduction_type', $data);
        return $this->db->insert_id();
    }
    ...
    {
        $this->db->where('id', (int) $id);
        $this->db->update(db_prefix() . 'hrm_deduction_type', $data);
        return $this->db->affected_rows() > 0;
    }
    ...
    {
        $this->db->where('id', (int) $id);
        $this->db->delete(db_prefix() . 'hrm_deduction_type');
        return $this->db->affected_rows() > 0;
    }

    /* ---------------------------------------------------------------------
     * v2.6.3 - Per-employee deduction / cash-advance ledger
     * ------------------------------------------------------------------- */
    public function get_staff_deduction($id = false)
    {
        $this->db->select('d.*, CONCAT(s.firstname, " ", s.lastname) as staff_name, t.name as type_name');
        $this->db->from(db_prefix() . 'hrm_staff_deduction d');
        $this->db->join(db_prefix() . 'staff s', 's.staffid = d.staff_id', 'left');
        $this->db->join(db_prefix() . 'hrm_deduction_type t', 't.id = d.deduction_type_id', 'left');
        if (is_numeric($id)) {
    ...
    public function add_staff_deduction($data)
    {
        ...
        $this->db->insert(db_prefix() . 'hrm_staff_deduction', $data);
        return $this->db->insert_id();
    }

    public function update_staff_deduction($data, $id)
    {
        if (isset($data['start_month'])) {
            $data['start_month'] = !empty($data['start_month']) ? to_sql_date($data['start_month']) : null;
        }
        $this->db->where('id', (int) $id);
        $this->db->update(db_prefix() . 'hrm_staff_deduction', $data);
        return $this->db->affected_rows() > 0;
    }

    public function delete_staff_deduction($id)
    {
        $this->db->where('deduction_id', (int) $id);
        $this->db->delete(db_prefix() . 'hrm_staff_deduction_collection');
        $this->db->where('id', (int) $id);
        $this->db->delete(db_prefix() . 'hrm_staff_deduction');
        return $this->db->affected_rows() > 0;
    }

    public function get_deduction_collections($deduction_id)
    {
        $this->db->where('deduction_id', (int) $deduction_id);
        $this->db->order_by('id', 'desc');
        return $this->db->get(db_prefix() . 'hrm_staff_deduction_collection')->result_array();
    }

    /**
     * Record a collection (installment/salary recovery) against a staff deduction
     * and roll the collected total + status forward on the parent record.
     */
    public function add_deduction_collection($data)
    {
        $deduction_id = (int) ($data['deduction_id'] ?? 0);
        $amount       = (float) ($data['amount'] ?? 0);
        if (!$deduction_id || $amount <= 0) {
            return false;
        }

        $deduction = $this->get_staff_deduction($deduction_id);
        if (!$deduction) {
            return false;
        }

        // Never collect more than the outstanding balance.
        $outstanding = (float) $deduction['total_amount'] - (float) $deduction['collected_amount'];
        if ($outstanding <= 0) {
            return false;
        }
        ...
        $collection = [
            ...
            'amount'         => $amount,
            'collected_date' => !empty($data['collected_date']) ? to_sql_date($data['collected_date']) : date('Y-m-d'),
            'period'         => $data['period'] ?? null,
            'notes'          => $data['notes'] ?? null,
            'add_from'       => get_staff_user_id(),
            'date_created'   => date('Y-m-d H:i:s'),
        ];
        $this->db->insert(db_prefix() . 'hrm_staff_deduction_collection', $collection);
        $insert_id = $this->db->insert_id();

        $new_collected = (float) $deduction['collected_amount'] + $amount;
        $update = ['collected_amount' => $new_collected];
        if ($new_collected >= (float) $deduction['total_amount']) {
            $update['status'] = 'completed';
        }
        $this->db->where('id', $deduction_id);
        $this->db->update(db_prefix() . 'hrm_staff_deduction', $update);
     ...
     * ------------------------------------------------------------------- */
    public function get_thirteenth_month($year = false)
    {
        $this->db->select('m.*, CONCAT(s.firstname, " ", s.lastname) as staff_name');
        $this->db->from(db_prefix() . 'hrm_thirteenth_month m');
        $this->db->join(db_prefix() . 'staff s', 's.staffid = m.staff_id', 'left');
        if ($year !== false && $year !== '' && $year !== null) {
            $this->db->where('m.year', (int) $year);
        }
        $this->db->order_by('m.year', 'desc');
        $this->db->order_by('staff_name', 'asc');
        return $this->db->get()->result_array();
    }

    public function get_thirteenth_month_row($id)
    {
        return $this->db->get_where(db_prefix() . 'hrm_thirteenth_month', ['id' => (int) $id])->row_array();
    }

    /**
     * 13th month = monthly base salary pro-rated by months worked in the year.
     */
    public function compute_thirteenth_month_amount($base_amount, $months_worked)
    {
        $base   = (float) $base_amount;
        $months = (float) $months_worked;
        if ($months < 0) $months = 0;
        if ($months > 12) $months = 12;
        return round($base * $months / 12, 2);
    }

    public function save_thirteenth_month($data, $id = '')
    {
        $data['base_amount']     = (float) ($data['base_amount'] ?? 0);
        $data['months_worked']   = (float) ($data['months_worked'] ?? 12);
        $data['computed_amount'] = $this->compute_thirteenth_month_amount($data['base_amount'], $data['months_worked']);

        if ($id) {
            $this->db->where('id', (int) $id);
            $this->db->update(db_prefix() . 'hrm_thirteenth_month', $data);
            return (int) $id;
        }
        $data['date_created'] = date('Y-m-d H:i:s');
        $this->db->insert(db_prefix() . 'hrm_thirteenth_month', $data);
        return $this->db->insert_id();
    }
    ...
    {
        $this->db->where('id', (int) $id);
        $this->db->delete(db_prefix() . 'hrm_thirteenth_month');
        return $this->db->affected_rows() > 0;
    }

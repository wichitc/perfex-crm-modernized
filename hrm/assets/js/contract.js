   $(function() {
       ...
       // Reload the templates dropdown when the contract type changes.
       $('select[name="name_contract"]').on('change', function () {
           var typeId = $(this).val();
           var $sel   = $('#template_id');
           if (!$sel.length) return;

           if (!typeId) {
               $sel.empty().append('<option value=""></option>').selectpicker('refresh');
               return;
           }

           $.getJSON(admin_url + 'hrm/get_contract_templates_by_type_ajax/' + encodeURIComponent(typeId), function (resp) {
               var current = $sel.data('current') || '';
               $sel.empty();
               $sel.append('<option value=""></option>');
               (resp.templates || []).forEach(function (t) {
                   var sel = String(t.id) === String(current) ? ' selected' : '';
                   $sel.append('<option value="' + t.id + '"' + sel + '>' + $('<div/>').text(t.name).html() + '</option>');
               });
               $sel.selectpicker('refresh');
           });
       });

       // When the user picks a template, push its content into the body editor
       // (TinyMCE if available, else the plain textarea).
       $(document).on('change', '#template_id', function () {
           var tplId = $(this).val();
           if (!tplId) return;
           // Only auto-replace if the body is empty — never overwrite user edits.
           var $ta = $('textarea[name="body_content"]');
           var current = '';
           if (typeof tinymce !== 'undefined' && tinymce.get('body_content')) {
               current = tinymce.get('body_content').getContent();
           } else {
               current = $ta.val();
           }
           if ($.trim(current.replace(/<p>(\s|&nbsp;)*<\/p>/gi, '')) !== '') {
               if (!confirm($ta.data('overwrite-confirm') || 'Replace current contract body with template content?')) {
                   return;
               }
           }
           var typeId = $('select[name="name_contract"]').val() || 0;
           $.getJSON(admin_url + 'hrm/get_contract_templates_by_type_ajax/' + encodeURIComponent(typeId), function (resp) {
               (resp.templates || []).forEach(function (t) {
                   if (String(t.id) === String(tplId)) {
                       if (typeof tinymce !== 'undefined' && tinymce.get('body_content')) {
                           tinymce.get('body_content').setContent(t.content || '');
                       } else {
                           $ta.val(t.content || '');
                       }
                   }
       ...
       // Click any merge-field token to copy it to the clipboard.

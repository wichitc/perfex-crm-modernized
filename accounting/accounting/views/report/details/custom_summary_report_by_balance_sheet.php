<?php 
        ...
        <tr>
          <td width="100%" colspan="<?php echo new_html_entity_decode($count); ?>">
        ...
        <tr class="tr_header">
          <td width="<?php echo new_html_entity_decode($rate_col); ?>%"></td>
          <?php 
          switch ($data_report['display_columns_by']) {
            case 'total_only':
              echo '<td width="'.$rate_col.'%" class="text-right text-bold">'. _l('total') .'</td>';
              break;
            ...
            case 'months':
              ...
              while($month <= $end)
              {
                echo '<td width="'.$rate_col.'%" class="text-right text-bold">'.date('F', $month).'<br>'.date('Y', $month).'</td>';
                  $month = strtotime("+1 month", $month);
              }
            ...
            case 'quarters':
              ...
              while (strtotime($from_date) < strtotime($to_date)) {
                  ...
                  echo '<td width="'.$rate_col.'%" class="text-right text-bold">'.$t.'</td>';
                  ...
                  if(strtotime($from_date) > strtotime($to_date)){
                      ...
                      if($month . ' - ' . $year != $month_2 . ' - ' . $year_2){
                          echo '<td width="'.$rate_col.'%" class="text-right text-bold">'.$t_2.'</td>';
                      }
                  }
              }
              echo '<td width="'.$rate_col.'%" class="text-right text-bold">'. _l('total') .'</td>';
            ...
            case 'years':
              ...
              while (strtotime($from_date) < strtotime($to_date)) {
                  ...
                  echo '<td width="'.$rate_col.'%" class="text-right text-bold">'.$year.'</td>';
                  ...
                  if(strtotime($from_date) > strtotime($to_date)){
                      ...
                      if($year != $year_2){
                          echo '<td width="'.$rate_col.'%" class="text-right text-bold">'.$year_2.'</td>';
                      }
                  }
              }
              echo '<td width="'.$rate_col.'%" class="text-right text-bold">'. _l('total') .'</td>';
            ...
            case 'vendors':
              ...
              $vendors = $this->purchase_model->get_vendor();
              foreach ($vendors as $key => $vendor) {
                  echo '<td width="'.$rate_col.'%" class="text-right text-bold">'.$vendor['company'].'</td>';
              }
              echo '<td width="'.$rate_col.'%" class="text-right text-bold">'. _l('not_specified') .'</td>';
              echo '<td width="'.$rate_col.'%" class="text-right text-bold">'. _l('total') .'</td>';
              break;
            ...
            case 'employees':
              ...
              $staffs = $this->staff_model->get();
              foreach ($staffs as $key => $staff) {
                echo '<td width="'.$rate_col.'%" class="text-right text-bold">'.$staff['full_name'].'</td>';
              }
              echo '<td width="'.$rate_col.'%" class="text-right text-bold">'. _l('total') .'</td>';
              break;
            ...
            case 'product_service':
              ...
              $items = $this->invoice_items_model->get();
              foreach ($items as $key => $item) {
                echo '<td class="text-right text-bold">'.$item['description'].'</td>';
              }
              echo '<td width="'.$rate_col.'%" class="text-right text-bold">'. _l('not_specified') .'</td>';
              echo '<td width="'.$rate_col.'%" class="text-right text-bold">'. _l('total') .'</td>';
              break;
            case 'customers':
              ...
              $clients = $this->clients_model->get();
              foreach ($clients as $key => $client) {
                echo '<td width="'.$rate_col.'%" class="text-right text-bold">'.$client['company'].'</td>';
              }
              echo '<td width="'.$rate_col.'%" class="text-right text-bold">'. _l('not_specified') .'</td>';
              echo '<td width="'.$rate_col.'%" class="text-right text-bold">'. _l('total') .'</td>';
              break;
        ...
        <?php
          ...
          ?>
          <tr data-node-id="<?php echo new_html_entity_decode($parent_index); ?>" data-node-pid="1000" class="parent-node expanded">
          ...
          <?php 
            $row_index += 1;
           ?>
          <tr data-node-id="<?php echo new_html_entity_decode($row_index); ?>" data-node-pid="<?php echo new_html_entity_decode($parent_index); ?>" class="parent-node expanded tr_total">
          ...
          <?php 
            ...
            $row_index += 1;
          ?>
          <tr data-node-id="<?php echo new_html_entity_decode($row_index); ?>" data-node-pid="1000" class="parent-node expanded tr_total">
          ...
          <?php 
            $row_index += 1;
            ?>
          <tr data-node-id="<?php echo new_html_entity_decode($row_index); ?>" clas="tr_total">
          ...
          <?php 
            $row_index += 1;
            ?>
            <tr data-node-id="1001" class="parent-node expanded">
            ...
            ?>
            <tr data-node-id="<?php echo new_html_entity_decode($parent_index); ?>" data-node-pid="1001" class="parent-node expanded">
            ...
            <?php 
              ...
              ?>
            <?php $row_index += 1; ?>
            <tr data-node-id="<?php echo new_html_entity_decode($row_index); ?>" data-node-pid="<?php echo new_html_entity_decode($parent_index); ?>" class="tr_total">
            ...
            <tr data-node-id="<?php echo new_html_entity_decode($row_index); ?>" data-node-pid="1001" class="tr_total">
            ...
            <?php $row_index += 1; ?>
            <tr data-node-id="<?php echo new_html_entity_decode($row_index); ?>" class="tr_total">

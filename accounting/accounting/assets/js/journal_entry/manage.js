  ...
  "use strict";
    if (confirm_delete()) {
        var ids = [],
            ...
            data.export_to_excel = $('#export_to_excel').prop('checked');
        ...
        setTimeout(function() {
            $.post(admin_url + 'accounting/journal_entry_bulk_action', data).done(function() {
              if($('#export_to_excel').prop('checked') === true){
                window.location.href = admin_url + 'accounting/download_journal_entry_files';
              }else{

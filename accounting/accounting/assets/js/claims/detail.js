...
document.addEventListener("DOMContentLoaded", function() {
  ...
  jQuery(document).on('click', '.btn-edit-claim-refund', function() {
    var $button = jQuery(this);
    var refundId = $button.data('id');
    var maxAmount = parseFloat($button.data('max')) || 0;
    var $form = jQuery('#edit-refund-form');

    $form.attr('action', $form.data('action-base') + refundId);
    jQuery('#edit_refund_amount')
      .val($button.data('amount'))
      .attr('max', maxAmount.toFixed(2));
    jQuery('.edit-refund-max-help').text((typeof app !== 'undefined' && app.lang && app.lang.acc_maximum_allowed_refund ? app.lang.acc_maximum_allowed_refund : 'Maximum allowed refund') + ': ' + maxAmount.toFixed(2));
    jQuery('#edit-refund-form #payment_date').val($button.data('payment-date'));
    jQuery('#edit_payment_method').val($button.data('payment-method')).selectpicker('refresh');
    jQuery('#edit_credit_account_id').val($button.data('credit-account-id')).selectpicker('refresh');
    jQuery('#edit_refund_notes').val($button.data('notes') || '');
    jQuery('#edit-refund-modal').modal('show');
  });
});
...
function sign_request_claim(id) {
  "use strict"; 
  if(jQuery('#upload').is(':checked')){
    if( document.getElementById("sign_attachment_file").files.length == 0 ){
      ...
      change_claim_approval_status(id, 2, true);
...
function approve_request_claim(id) {
  "use strict"; 
  change_claim_approval_status(id, 2);
  if( document.getElementById("sign_attachment_file") && document.getElementById("sign_attachment_file").files.length > 0 ){
    jQuery('#sign_attachment-form').submit();
  }
}

function deny_request_claim(id) {
  "use strict"; 
  change_claim_approval_status(id, 3);
  if( document.getElementById("sign_attachment_file") && document.getElementById("sign_attachment_file").files.length > 0 ){
    jQuery('#sign_attachment-form').submit();
  }
}

function change_claim_approval_status(id, status, sign_code = false) {
  var data = {};
  data.rel_id = id;
  data.rel_type = 'claim';
  data.approve = status;

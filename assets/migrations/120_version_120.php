...
class Migration_Version_120 extends App_module_migration
{
    public function up()
    {
        ...
        // Fix supplier_phone column type (was INT, should be VARCHAR)
        ...
        if (!$CI->db->field_exists('barcode', db_prefix().'assets')) {
            $CI->db->query('ALTER TABLE `'.db_prefix().'assets` 
                ...
                ADD `created_at` DATETIME NULL AFTER `maintenance_interval_days`,
                ADD `updated_at` DATETIME NULL AFTER `created_at`,
                ADD `created_by` INT(11) NULL AFTER `updated_at`,
        ...
        $CI->db->query('ALTER TABLE `'.db_prefix().'assets` ADD INDEX IF NOT EXISTS `status` (`status`)');
        $CI->db->query('ALTER TABLE `'.db_prefix().'assets` ADD INDEX IF NOT EXISTS `asset_group` (`asset_group`)');
        $CI->db->query('ALTER TABLE `'.db_prefix().'assets` ADD INDEX IF NOT EXISTS `department` (`department`)');
        $CI->db->query('ALTER TABLE `'.db_prefix().'inventory_history` ADD INDEX IF NOT EXISTS `assets` (`assets`)');
    }

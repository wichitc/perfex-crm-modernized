...
"use strict";
	...
	$('.star-rating .fa-star').on('click', function() {
	   $('.star-rating .fa-star').siblings('input[name="rating['+$(this).data('id')+']"]').val($(this).data('rating'));
...
function sendmail(){
	...
	$('#care_modal').modal({show: true,backdrop: 'static'});
...
function open_rating_dialog(){
	"use strict";
	$('#candidate_rating').modal({show: true,backdrop: 'static'});
}

function re_status_mark_as(status, task_id, type) {
	"use strict"; 

	var url = 'recruitment/re_status_mark_as/' + status + '/' + task_id + '/' + type;

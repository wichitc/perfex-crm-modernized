...
function new_proposal(){
  "use strict";
  $('#recruitment_proposal').modal({show: true,backdrop: 'static'});
  ...
  /*get job_description*/
  $.post(admin_url + 'recruitment/get_recruitment_proposal_add').done(function(response) {
...
function edit_proposal(invoker,id){
  ...
  $('#recruitment_proposal select[name="experience"]').change();
  $('#recruitment_proposal select[name="currency"]').val($(invoker).data('currency'));
  $('#recruitment_proposal select[name="currency"]').change();

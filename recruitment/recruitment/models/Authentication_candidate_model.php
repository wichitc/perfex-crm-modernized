...
class Authentication_candidate_model extends App_Model
{
        ...
        if ((!empty($email)) and (!empty($password))) {
            ...
            if ($user) {
                ...
                if (!app_hasher()->CheckPassword($password, $user->password ?? '')) {
                    hooks()->do_action('candidate_failed_login_attempt', [
                        'user'            => $user,
                        'is_staff_member' => $staff,
                    ]);

                    log_activity('Failed Login Attempt [Email: ' . $email . ', Is Candidate: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');

                    return false;
                }
            } else {

                hooks()->do_action('candidate_non_existent_user_login_attempt', [
                        'email'           => $email,
                        'is_staff_member' => $staff,
                ]);

                log_activity('Non Existing User Tried to Login [Email: ' . $email . ', Is Candidate: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');

                return false;
            }

            if ($user->active == 0) {
                hooks()->do_action('candidate_inactive_user_login_attempt', [
                        'user'            => $user,
                        'is_staff_member' => $staff,
                ]);
                log_activity('Inactive User Tried to Login [Email: ' . $email . ', Is Candidate: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');
            ...
            if ($staff == true) {
                ...
                if (!$twoFactorAuth) {
                    hooks()->do_action('candidate_before_staff_login', [
                        'email'  => $email,
                        'userid' => $user->$_id,
                    ]);
                ...
                hooks()->do_action('candidate_before_vendor_login', [
                    'email'           => $email,
    ...
    public function logout($staff = true)
    {
        ...
        if (is_candidate_logged_in()) {
            hooks()->do_action('candidate_before_contact_logout', get_candidate_id());

            $this->session->unset_userdata('candidate_id');
            $this->session->unset_userdata('candidate_logged_in');
        } else {
            hooks()->do_action('candidate_before_staff_logout', get_staff_user_id());
    ...
    {
        $this->load->helper('cookie');
        if ($cookie = get_cookie('candidate_autologin', true)) {
            $data = unserialize($cookie);
            $this->user_autologin->delete($data['user_id'], md5($data['key']), $staff);
            delete_cookie('candidate_autologin', 'aal');
        }
    }
    ...
    {
        if (!is_candidate_logged_in() && !is_staff_logged_in()) {
            $this->load->helper('cookie');
            if ($cookie = get_cookie('candidate_autologin', true)) {
                $data = unserialize($cookie);
                if (isset($data['key']) and isset($data['user_id'])) {
                    if (!is_null($user = $this->user_autologin->get($data['user_id'], md5($data['key'])))) {
                        ...
                        if ($user->staff == 1) {
                            ...
                            $contact = $this->db->get(db_prefix() . 'rec_candidate')->row();
    ...
    /**
     * Send set password email for rec_candidate
     * @param string $email
     */
    public function set_password_email($email)
    {
        $this->db->where('email', $email);
        $user = $this->db->get(db_prefix() . 'rec_candidate')->row();
        ...
        if ($user) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                if ($sent) {
                    hooks()->do_action('candidate_set_password_email_sent', ['is_staff_member' => false, 'user' => $user]);
    ...
    public function forgot_password($email, $staff = false)
    {
        ...
        if ($user) {
            ...
            if ($this->db->affected_rows() > 0) {
                ...
                $html = '';
                $html .= _l('re_Create_New_Password').' <br>';
                $html .= _l('re_Forgot_your_password').' <br>';
                $html .= _l('re_To_create_a_new_password_just_follow_this_link').' <br><br>';
                $html .= '<a href="'.site_url('recruitment/authentication_candidate/reset_password/0/'.$user->$_id.'/'.$new_pass_key).'">'._l('re_Reset_Password').'</a><br><br>';

                $html .= _l('re_You_received_this_email_because_it_was_requested_by_a').' '.get_option('companyname').'. '._l('re_This_is_part_of_the_procedure_to_create');

                $sent = $this->emails_model->send_simple_email($user->email, _l('re_Create_New_Password'), $html);

                if ($sent) {
                    hooks()->do_action('candidate_forgot_password_email_sent', ['is_staff_member' => $staff, 'user' => $user]);
    ...
    public function set_password($staff, $userid, $new_pass_key, $password)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            log_activity('User Set Password [User ID: ' . $userid . ', Is Candidate: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');
    ...
    public function reset_password($staff, $userid, $new_pass_key, $password)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            log_activity('User Reseted Password [User ID: ' . $userid . ', Is Candidate: ' . ($staff == true ? 'Yes' : 'No') . ', IP: ' . $this->input->ip_address() . ']');
            $this->db->set('new_pass_key', null);
            $this->db->set('new_pass_key_requested', null);
            $this->db->set('last_password_change', date('Y-m-d H:i:s'));
            $this->db->where($_id, $userid);

<div class="panel_s">
	<div class="panel-body">
		<?php if ($campaigns->cp_status == 1) {?>
			<div class="ribbon warnings hide"><span><?php echo _l('planning'); ?></span></div>
		<?php } elseif ($campaigns->cp_status == 3) {?>
			<div class="ribbon info hide"><span><?php echo _l('in_progress'); ?></span></div>
		<?php } elseif ($campaigns->cp_status == 4) {?>
			<div class="ribbon success hide"><span><?php echo _l('finish'); ?></span></div>
		<?php } elseif ($campaigns->cp_status == 5) {?>
			<div class="ribbon danger hide"><span><?php echo _l('cancel'); ?></span></div>
		<?php }?>
		<?php if (($campaigns->cp_status == 1 || $campaigns->cp_status == 2) && date('Y-m-d') > $campaigns->cp_to_date) {?>
			<div class="ribbon warning hide"><span><?php echo _l('overdue'); ?></span></div>
		<?php }?>
		<div class="row col-md-12">
			<div class="col-md-9">
				<h4 class="general-infor-color"><?php echo _l('general_infor') ?></h4>
			</div>
			<?php $manager = new_explode(',', $campaigns->cp_manager);?>
		...
		$current_id = $campaigns->currency;
		if($campaigns->currency == 0){
			$current_id= $get_base_currency->id;
		}

		$manager = new_explode(',', $campaigns->cp_manager); 
		$curent_user = get_staff_user_id();
		if((in_array($curent_user, $manager) || $curent_user == $campaigns->cp_add_from || is_admin()) ){
			?>

			<div class=" col-md-3 pull-right">
				<select name="change_status" id="change_status" class="selectpicker dropdown bootstrap-select show-tick bs3" onchange="change_status_campaign(this,<?php echo new_html_entity_decode($campaigns->cp_id); ?>); return false;" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('change_status_to'); ?>">
				...
				<table class="table border table-striped margin-top-0">
					<tbody>
						<tr class="project-overview">
							...
							<td><?php echo new_html_entity_decode($campaigns->campaign_code); ?></td>
						</tr>
						<tr class="project-overview">
							<td class="bold" width="30%"><?php echo _l('campaign_name'); ?></td>
							<td><?php echo new_html_entity_decode($campaigns->campaign_name); ?></td>
						</tr>
						<tr class="project-overview">
						...
						<tr class="project-overview">
							...
							?>
							<td><?php echo new_html_entity_decode($r_form_name); ?></td>
						</tr>

						<tr class="project-overview">
							<td class="bold"><?php echo _l('reason_recruitment'); ?></td>
							<td><?php echo new_html_entity_decode($campaigns->cp_reason_recruitment); ?></td>
						</tr>
						<tr class="project-overview">
							<td class="bold"><?php echo _l('company_name'); ?></td>
							<td><?php echo new_html_entity_decode(get_rec_company_name($campaigns->company_id)); ?></td>
						</tr>
							...
							<tr class="project-overview">
								...
								<td> 
									<span class="label label-default">
										<a href="<?php echo site_url('recruitment/forms/wtl/'.$campaigns->cp_id.'/'.$rec_channel); ?>" target="_blank"><?php echo site_url('recruitment/forms'); ?></a>
									</span>
								</tr>
							<?php } ?>
					...
					<table class="table table-striped margin-top-0">
						<tbody>
							...
							<tr class="project-overview">
								...
								<td><?php echo new_html_entity_decode($campaigns->cp_amount_recruiment); ?></td>
							</tr>
							<tr class="project-overview">
								<td class="bold"><?php echo _l('workplace'); ?></td>
								<td><?php echo new_html_entity_decode($campaigns->cp_workplace); ?></td>
							</tr>
							...
							<tr class="project-overview">
								...
								<td>
									<?php $follows = new_explode(',', $campaigns->cp_follower);
							...
							<table class="table border table-striped margin-top-0">
								<tbody>
									...
									<tr class="project-overview">
										...
										<td><?php echo new_html_entity_decode($campaigns->cp_height); ?></td>
									</tr>
									<tr class="project-overview">
							...
							<table class="table table-striped margin-top-0">
								<tbody>
									<tr class="project-overview">
										...
										<td><?php echo new_html_entity_decode($campaigns->cp_ages_from . ' - ' . $campaigns->cp_ages_to); ?></td>
									</tr>
									<tr class="project-overview">
										<td class="bold"><?php echo _l('weight'); ?></td>
										<td><?php echo new_html_entity_decode($campaigns->cp_weight); ?></td>
									</tr>
									<tr class="project-overview">
						...
						<div class="col-md-12">
							<div id="p_custom_fields_items">
								<?php echo render_custom_fields('campaign', $campaigns->cp_id); ?>
							</div>
						</div>

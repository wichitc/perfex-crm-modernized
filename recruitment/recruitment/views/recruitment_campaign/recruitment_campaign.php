				...
				<div class="panel_s">
					<div class="panel-body">
						...
						<div class="row">
								...
								<div class="form-group">
									<select name="department_filter[]" id="department_filter" class="selectpicker" multiple="true" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('filter_by_department'); ?>">
										...
										<?php foreach ($departments as $s) {?>
											<option value="<?php echo new_html_entity_decode($s['departmentid']); ?>"><?php echo new_html_entity_decode($s['name']); ?></option>
										<?php }?>
									</select>
								</div>
							</div>
								...
								<div class="form-group">
									<select name="position_filter[]" id="position_filter" class="selectpicker" data-live-search="true" multiple="true" data-width="100%" data-none-selected-text="<?php echo _l('filter_by_position'); ?>">
										...
										<?php foreach ($positions as $s) {?>
											<option value="<?php echo new_html_entity_decode($s['position_id']); ?>"><?php echo new_html_entity_decode($s['position_name']); ?></option>
										<?php }?>
									</select>
								</div>
							</div>
							...
							<div class="col-lg-3 ">
								<div class="form-group">
									<select name="company_filter[]" id="company_filter" class="selectpicker" data-live-search="true" multiple="true" data-width="100%" data-none-selected-text="<?php echo _l('company_name'); ?>">
							...
							<div class="col-md-3"> <?php echo render_date_input('cp_from_date_filter', 'from_date', ''); ?></div>
							<div class="col-md-3"> <?php echo render_date_input('cp_to_date_filter', 'to_date', ''); ?></div>

							<div class="col-md-3">
								<div class="form-group">
									<label for="rec_channel_form_id_filter"><?php echo _l('recruitment_channel_form'); ?></label>
									<select name="rec_channel_form_id_filter[]" id="rec_channel_form_id_filter" class="selectpicker"  data-live-search="true" multiple="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>">
										<?php foreach ($rec_channel_form as $rec_c_f) {?>
											<option value="<?php echo new_html_entity_decode($rec_c_f['id']); ?>"><?php echo new_html_entity_decode($rec_c_f['r_form_name']); ?></option>
										<?php }?>
									</select>
								</div>
							</div>
							...
							<div class="col-md-3">
								<label for="cp_manager_filter"><?php echo _l('manager'); ?></label>
								<select name="cp_manager_filter[]" id="cp_manager_filter" class="selectpicker" multiple="true" data-actions-box="true" data-live-search="true" multiple="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>">
						...
						$table_data = array(
							...
							_l('form_of_work'),
							_l('department'),
							_l('recruiment_duration'),
							_l('amount_recruiment'),
							_l('recruitment_channel_form'),
							_l('manager'),
							_l('status'),
						);
						$cf = get_custom_fields('campaign',array('show_on_table'=>1));
						foreach($cf as $custom_field) {
							array_push($table_data,$custom_field['name']);
						}
						?>
						<?php render_datatable($table_data, 'table_rec_campaign', ['campaign_sm' => 'campaign_sm']);?>
					</div>
				</div>
			</div>
				...
				<div class="tab-content">
						...
						<div class="col-md-12 <?php  if(get_recruitment_option('recruitment_create_campaign_with_plan') != 1 ){ echo 'hide';} ;?>">
							...
							<select name="cp_proposal[]" id="proposal" class="selectpicker" multiple="true" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>">
								...
								<?php foreach ($rec_proposal as $s) {?>
									<option value="<?php echo new_html_entity_decode($s['id']); ?>"><?php echo new_html_entity_decode($s['proposal_name']); ?></option>
								<?php }?>
							</select>
							<br><br>
						</div>
							...
							<div class="form-group">
								...
								<select  name="company_id" id="company_id" class="selectpicker" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>">
									...
									<?php foreach ($company_list as $company) {?>
										<option value="<?php echo new_html_entity_decode($company['id']); ?>"><?php echo new_html_entity_decode($company['company_name']); ?></option>
									<?php }?>
								</select>
							</div>
						</div>
					...
					<div class="col-md-2">
						...
						foreach ($currencies as $currency) {
							...
							if (isset($recruitment_proposal)) {
								if ($currency['id'] == $recruitment_proposal->currency) {
						...
						<?php echo render_select('currency', $currencies, ['id', 'name', 'symbol'], 'invoice_add_edit_currency', '', $currency_attr, [],  '', '', false); ?>
					</div>
					...
					<div class="col-md-12">
						<?php echo render_input('job_meta_title', 'rec_job_meta_title'); ?>
					</div>
					<div class="col-md-12">
						<?php echo render_textarea('job_meta_description', 'rec_job_meta_description'); ?>
					</div>

					 <!-- render custom fields -->
                    <div class="col-md-12" id="custom_fields_items">
                      <?php echo render_custom_fields('campaign'); ?>
                    </div>

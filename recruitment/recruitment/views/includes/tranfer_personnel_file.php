         ...
         <div class="modal-body">
            <div class="row">
               <div class="col-md-12 border-right project_file_area">
                  ...
                  <?php } ?>
                  <?php
                     $path = RECRUITMENT_MODULE_UPLOAD_FOLDER.'/set_transfer/' .$file->rel_id.'/'.$file->file_name;
                     if(is_image($path)){ ?>
                  <img src="<?php echo base_url('modules/recruitment/uploads/set_transfer/'.$file->rel_id.'/'.$file->file_name); ?>" class="img img-responsive" style="margin: auto">
                  ...
                  <iframe src="<?php echo base_url('modules/recruitment/uploads/set_transfer/'.$file->rel_id.'/'.$file->file_name); ?>" height="100%" width="100%" frameborder="0"></iframe>
                  <?php } else if(strpos($file->filetype,'xls') !== false && empty($file->external)){ ?>
                  <iframe src='https://view.officeapps.live.com/op/embed.aspx?src=<?php echo base_url('modules/recruitment/uploads/set_transfer/'.$file->rel_id.'/'.$file->file_name); ?>' width='100%' height='100%' frameborder='0'>
                  </iframe>
                  ...
                  <?php } else if(is_markdown_file($path) && $previewMarkdown = markdown_parse_preview($path)) {
                     ...
                     echo '<a href="'.site_url('modules/recruitment/uploads/set_transfer/'.$file->rel_id.'/'.$file->file_name).'" download>'.$file->file_name.'</a>';

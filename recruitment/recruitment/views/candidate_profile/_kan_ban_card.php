...
<li data-task-id="<?php echo new_html_entity_decode($candidate['id']); ?>">
  <div class="panel-body">
    <div class="row">
      <div class="col-md-12 task-name">

        <a href="<?php echo admin_url('recruitment/candidate/' . $candidate['id']); ?>" >
          <span class="inline-block full-width mtop10 mbot10"><?php echo new_html_entity_decode($candidate['candidate_code'] .'_'.$candidate['candidate_name'].' '.$candidate['last_name']); ?></span>
        </a>
      </div>
     ...
     <div class="col-md-10 text-left text-muted">
        <span class="mright5 inline-block text-muted" data-toggle="tooltip" >
          <?php echo new_html_entity_decode($candidate['phonenumber']) ?>
        </span>
        <span class="mright5 inline-block text-muted" data-toggle="tooltip" >
         <?php echo new_html_entity_decode($candidate['email']) ?>
        </span>
        <span class="mright5 inline-block text-muted" data-toggle="tooltip" >
          <?php
          $cp = get_rec_campaign_hp($candidate['rec_campaign']);
          $datas = '';
          if (!is_null($candidate['rec_campaign']) && isset($cp)) {

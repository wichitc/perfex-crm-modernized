...
class Invoices_model extends App_Model {
	...
	public function get_items_by_type($type, $rel_id, $playground = false) {
		$this->db->where('rel_type', $type);
		$this->db->where('rel_id', $rel_id);
		$this->db->order_by('item_order', 'ASC');
		
		$items = $this->db->get(db_prefix() . ($playground ? 'playground_' : '') . 'itemable')->result_array();
		...
		foreach ($items as $item) {
			...
			$this->db->where('rel_type', $type);
			$taxes = $this->db->get(db_prefix() . ($playground ? 'playground_' : '') . 'item_tax')->result_array();
			
			$items[$i]['taxname'] = [];
			$items[$i]['taxrate'] = [];
			
			foreach ($taxes as $tax) {
				array_push($items[$i]['taxname'], $tax['taxname']);
				array_push($items[$i]['taxrate'], $tax['taxrate']);
			}

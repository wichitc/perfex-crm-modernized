...
class Milestones extends REST_Controller {
    ...
    public function data_search_get($key = '') {
        ...
        // This allows: /api/milestones/search/term OR /api/milestones/search?q=term
        ...
        if (empty($key)) {
            $this->response(['status' => FALSE, 'message' => 'Search term is required. Use /api/milestones/search/{term} or /api/milestones/search?q={term}'], REST_Controller::HTTP_BAD_REQUEST);
            return;
        }
    ...
    public function data_post() {
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			if ($output > 0 && !empty($output)) {
				// success
				$this->api_fire_event('milestone_created', ['id' => $output]);
				$message = array(
					'status' => TRUE,
					'message' => 'Milestone add successful.',
    ...
    public function data_put($id = '') {
        ...
        if (empty($id) && !is_numeric($id)) {
            ...
            $update_data = $this->api_sanitize_update_payload('milestones', $update_data, ['custom_fields']);
            // update data
            $this->load->model('projects_model');
            $output = $this->projects_model->update_milestone($update_data, $id);
            if ($output > 0 && !empty($output)) {
                // success
                $this->api_fire_event('milestone_updated', ['id' => $id]);

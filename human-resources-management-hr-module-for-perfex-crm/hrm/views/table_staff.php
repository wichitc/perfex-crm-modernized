...
if(isset($department_id) && strlen($department_id) > 0){
    ...
    $dept_subquery = 'SELECT departmentid FROM (SELECT * FROM '.db_prefix().'departments ORDER BY '.db_prefix().'departments.parent_id, '.db_prefix().'departments.departmentid) departments_sorted, (SELECT @pv := '.$dept_id.') initialisation WHERE find_in_set(parent_id, @pv) AND length(@pv := concat(@pv, ",", departmentid)) OR departmentid = '.$dept_id;
    $where[] = ' AND '.db_prefix().'staff.staffid IN (SELECT staffid FROM '.db_prefix().'staff_departments WHERE departmentid IN ('.$dept_subquery.'))';
}

if($this->ci->input->post('status_work')){
    $allowed_status = ['working', 'inactivity', 'maternity_leave'];
    $where_status = '';
    $status = $this->ci->input->post('status_work');
        foreach ($status as $statues) {
            $statues = trim((string)$statues);
            if($statues !== '' && in_array($statues, $allowed_status, true))
            {
                $escaped = $this->ci->db->escape_str($statues);
                if($where_status == ''){
                    $where_status .= ' status_work = "'.$escaped.'"';
                }else{
                    $where_status .= ' or status_work = "'.$escaped.'"';
                }
            }
        }
        if($where_status != '')
...
if($this->ci->input->post('staff_role')){
    ...
    $staff_role      = $this->ci->input->post('staff_role');
        foreach ($staff_role as $staff_id) {
            $role_id = (int) $staff_id;
            if($role_id > 0)
            {
                if($where_role == ''){
                    $where_role .= '( '.db_prefix().'staff.role = '.$role_id;
                }else{
                    $where_role .= ' or '.db_prefix().'staff.role = '.$role_id;
                }
            }
        }
...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        elseif($aColumns[$i] == 'staff_identifi'){
            $_data = htmlspecialchars((string)($aRow['staff_identifi'] ?? ''), ENT_QUOTES, 'UTF-8');
       ...
       elseif($aColumns[$i] == 'sex'){
        ...
        elseif ($aColumns[$i] == 'active') {
            ...
            $_data .= ' <a href="' . admin_url('hrm/member/' . $aRow[db_prefix().'staff.staffid']) . '">' . htmlspecialchars($aRow['firstname'] . ' ' . $aRow['lastname'], ENT_QUOTES, 'UTF-8') . '</a>';
            ...
            $_data = '<a href="mailto:' . htmlspecialchars($_data ?? '', ENT_QUOTES, 'UTF-8') . '">' . htmlspecialchars($_data ?? '', ENT_QUOTES, 'UTF-8') . '</a>';
            ...
            if($aRow[db_prefix().'staff_departments.departmentid'] != ''){
                ...
                foreach ($team as $value) {
                    $j++;
                    $str .= '<span class="label label-tag tag-id-1"><span class="tag">'.htmlspecialchars($value['name'] ?? '', ENT_QUOTES, 'UTF-8').'</span><span class="hide">, </span></span>&nbsp';
            ...
            if (strpos($aColumns[$i], 'date_picker_') !== false) {
                $_data = (strpos((string)($_data ?? ''), ' ') !== false ? _dt($_data ?? '') : _d($_data ?? ''));
            }
        }
        $row[] = $_data;
    }

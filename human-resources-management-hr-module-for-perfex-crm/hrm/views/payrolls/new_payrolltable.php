                        ...
                        <div class="col-md-12">
                         <div class="form" id="new_payslip">
                            ...
                            <div class="row mtop15">
                                <div class="col-md-12">
                                    <div id="hot" style="min-height: 400px;"></div>
                                    <div class="mtop15">
                                        <button type="button" id="export-file" class="btn btn-default"><i class="fa fa-download"></i> <?php echo _l('dowload_payslip'); ?> (CSV)</button>
                                        <button type="button" id="print-payslip" class="btn btn-default" onclick="window.print();"><i class="fa fa-print"></i> <?php echo _l('print'); ?></button>
                                    </div>
                                </div>
                            </div>
    ...
    try {
      ...
      if (exportPlugin1) {
        exportPlugin1.downloadFile('csv', {
          ...
          fileExtension: 'csv',
          filename: '<?php echo _l('payslip'); ?>_' + new Date().toISOString().slice(0,10),
          mimeType: 'text/csv',
          rowDelimiter: '\r\n',
          rowHeaders: false
        });
      } else {
        var csv = Handsontable.helper.stringify(hot.getData());
        var blob = new Blob([csv], {type: 'text/csv;charset=utf-8;'});
        var link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.download = 'payslip_' + new Date().toISOString().slice(0,10) + '.csv';
        link.click();
        URL.revokeObjectURL(link.href);
      }
    } catch(e) {
      alert_float('warning', '<?php echo _l('dowload_payslip'); ?>: ' + (e.message || 'Error'));
    }
  });
}

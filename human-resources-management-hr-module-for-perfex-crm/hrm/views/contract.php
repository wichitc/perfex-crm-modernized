...
<div class="content">
   <div class="row">
            ...
            <div class="panel-body">
              <!-- start tab -->
               ...
               <li role="presentation" >
                ...
                <li role="presentation">
                   <a href="#tab_contract_document" aria-controls="tab_contract_document" role="tab" data-toggle="tab">
                   <span class="glyphicon glyphicon-list-alt"></span>&nbsp;<?php echo _l('document'); ?>
                   </a>
                </li>
              </ul>
              <div class="tab-content">
                ...
                <div role="tabpanel" class="tab-pane" id="tab_signed_information">
                    ...
                    <?php if (isset($contracts) && !empty($contracts[0]['signed_at'])): ?>
                    <hr>
                    <div class="row">
                      <div class="col-md-12">
                        <div class="alert alert-success">
                          <strong><?php echo _l('contract_signed_label'); ?>:</strong>
                          <?php echo _dt($contracts[0]['signed_at']); ?>
                          <?php if (!empty($contracts[0]['signed_ip'])) {
                              echo ' <span class="text-muted">(IP: ' . htmlspecialchars($contracts[0]['signed_ip']) . ')</span>';
                          } ?>
                        </div>
                        <?php if (!empty($contracts[0]['signature_image'])): ?>
                          <img src="<?php echo htmlspecialchars($contracts[0]['signature_image']); ?>"
                               alt="signature"
                               style="max-height:120px;border:1px solid #eee;background:#fff;padding:6px;">
                        <?php endif; ?>
                      </div>
                    </div>
                    <?php endif; ?>
                ...
                <!-- start: contract document tab -->
                <div role="tabpanel" class="tab-pane" id="tab_contract_document">
                    <div class="col-md-12">
                      <h4 class="no-margin"><?php echo _l('contract_document'); ?></h4>
                      <p class="text-muted"><?php echo _l('contract_document_help'); ?></p>
                      <hr class="hr-panel-heading">

                      <?php
                          $current_template_id = isset($contracts[0]['template_id']) ? (int) $contracts[0]['template_id'] : 0;
                          $current_body        = isset($contracts[0]['body_content']) ? (string) $contracts[0]['body_content'] : '';
                          $templates_for_type  = isset($contract_templates_for_type) ? (array) $contract_templates_for_type : [];
                      ?>

                      <div class="row">
                        <div class="col-md-6">
                          <label for="template_id" class="control-label"><?php echo _l('contract_template'); ?></label>
                          <select name="template_id" id="template_id" class="selectpicker" data-width="100%"
                                  data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>"
                                  data-current="<?php echo (int) $current_template_id; ?>">
                            <option value=""><?php echo _l('no_template'); ?></option>
                            <?php foreach ($templates_for_type as $tpl) { ?>
                              <option value="<?php echo (int) $tpl['id']; ?>" <?php echo ($current_template_id === (int) $tpl['id']) ? 'selected' : ''; ?>>
                        ...
                        <div class="col-md-6">
                          ...
                          <div class="hrm-merge-fields-cheat" style="max-height:120px;overflow:auto;border:1px solid #eee;padding:8px;background:#fafafa;">
                            <?php foreach ((array) ($merge_fields_definition ?? []) as $group) { ?>
                              <strong><?php echo htmlspecialchars($group['label']); ?>:</strong>
                              <?php foreach ($group['fields'] as $token => $label) { ?>
                                <code class="hrm-mf-token" style="cursor:pointer;margin:2px;display:inline-block;"
                                      data-token="<?php echo htmlspecialchars($token); ?>"
                                      title="<?php echo htmlspecialchars($label); ?>"><?php echo htmlspecialchars($token); ?></code>
                              <?php } ?>
                              <br>
                            <?php } ?>
                      ...
                      <br>
                      <?php echo render_textarea('body_content', _l('contract_body'), $current_body, ['rows' => 18, 'class' => 'tinymce']); ?>
                    </div>
                </div>
                <!-- end: contract document tab -->
      ...
      <div class="btn-bottom-toolbar text-right btn-toolbar-container-out">
         <?php if (isset($contracts) && !empty($contracts[0]['id_contract'])): ?>
            <a href="<?php echo admin_url('hrm/contract_view/' . (int) $contracts[0]['id_contract']); ?>" target="_blank" class="btn btn-default">
               <i class="fa fa-eye"></i> <?php echo _l('view_contract'); ?>
            </a>
            <a href="<?php echo admin_url('hrm/contract_view/' . (int) $contracts[0]['id_contract']); ?>?print=1" target="_blank" class="btn btn-default">
               <i class="fa fa-download"></i> <?php echo _l('download_contract'); ?>
            </a>
            <?php if (empty($contracts[0]['signed_at']) && hrm_can_sign_contract($contracts[0])): ?>
               <a href="<?php echo admin_url('hrm/contract_view/' . (int) $contracts[0]['id_contract']); ?>" target="_blank" class="btn btn-warning">
                  <i class="fa fa-pencil"></i> <?php echo _l('sign_contract'); ?>
               </a>
            <?php endif; ?>
         <?php endif; ?>
